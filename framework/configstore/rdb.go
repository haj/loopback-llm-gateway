...
// GetGuardrailConfigs returns a filtered, paginated page of guardrail configs
// plus the total matching count. Follows GetModelConfigsPaginated.
func (s *RDBConfigStore) GetGuardrailConfigs(ctx context.Context, params GuardrailConfigsQueryParams) ([]tables.TableGuardrailConfig, int64, error) {
	baseQuery := s.DB().WithContext(ctx).Model(&tables.TableGuardrailConfig{})
	...
	var configs []tables.TableGuardrailConfig
	if err := baseQuery.
...
// GetGuardrailConfig retrieves a single guardrail config by ID.
func (s *RDBConfigStore) GetGuardrailConfig(ctx context.Context, id string) (*tables.TableGuardrailConfig, error) {
	var config tables.TableGuardrailConfig
...
// GetEnabledGuardrailConfigs returns every enabled guardrail config. Used by the
// HTTP layer to rebuild the live loopback-guard guardrail list after a mutation.
func (s *RDBConfigStore) GetEnabledGuardrailConfigs(ctx context.Context) ([]tables.TableGuardrailConfig, error) {
	var configs []tables.TableGuardrailConfig
	if err := s.DB().WithContext(ctx).
		Where("enabled = ?", true).
...
// CreateGuardrailConfig inserts a new guardrail config.
func (s *RDBConfigStore) CreateGuardrailConfig(ctx context.Context, config *tables.TableGuardrailConfig, tx ...*gorm.DB) error {
	txDB := s.DB()
	if len(tx) > 0 {
		txDB = tx[0]
	}
	if err := txDB.WithContext(ctx).Create(config).Error; err != nil {
		return s.parseGormError(err)
	}
	return nil
}

// UpdateGuardrailConfig saves an existing guardrail config (full overwrite of
// mutable fields). The caller is expected to have loaded the row first.
func (s *RDBConfigStore) UpdateGuardrailConfig(ctx context.Context, config *tables.TableGuardrailConfig, tx ...*gorm.DB) error {
	txDB := s.DB()
	if len(tx) > 0 {
		txDB = tx[0]
	}
	if err := txDB.WithContext(ctx).Save(config).Error; err != nil {
		return s.parseGormError(err)
	}
	return nil
}

// DeleteGuardrailConfig deletes a guardrail config by ID.
func (s *RDBConfigStore) DeleteGuardrailConfig(ctx context.Context, id string, tx ...*gorm.DB) error {
	txDB := s.DB()
	if len(tx) > 0 {
		txDB = tx[0]
	}
	var config tables.TableGuardrailConfig
	if err := txDB.WithContext(ctx).Where("id = ?", id).First(&config).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return ErrNotFound
		}
		return err
	}
	return txDB.WithContext(ctx).Delete(&config).Error
}

// ----- Circuit breaker config CRUD -----

// GetCircuitBreakerConfigs returns every circuit breaker policy, newest first.
// Used by the HTTP layer to list policies and to re-apply enabled ones to the
// engine on startup. Follows GetEnabledGuardrailConfigs.
func (s *RDBConfigStore) GetCircuitBreakerConfigs(ctx context.Context) ([]tables.TableCircuitBreakerConfig, error) {
	var configs []tables.TableCircuitBreakerConfig
	if err := s.DB().WithContext(ctx).
		Order("created_at DESC, id DESC").
		Find(&configs).Error; err != nil {
		return nil, err
...
// GetCircuitBreakerConfig retrieves a single circuit breaker policy by ID.
func (s *RDBConfigStore) GetCircuitBreakerConfig(ctx context.Context, id string) (*tables.TableCircuitBreakerConfig, error) {
	var config tables.TableCircuitBreakerConfig
	if err := s.DB().WithContext(ctx).First(&config, "id = ?", id).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, ErrNotFound
		}
...
// GetCircuitBreakerConfigByProvider retrieves the policy for a provider, if any.
func (s *RDBConfigStore) GetCircuitBreakerConfigByProvider(ctx context.Context, provider string) (*tables.TableCircuitBreakerConfig, error) {
	var config tables.TableCircuitBreakerConfig
	if err := s.DB().WithContext(ctx).First(&config, "provider = ?", provider).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, ErrNotFound
		}
		return nil, err
...
// CreateCircuitBreakerConfig inserts a new circuit breaker policy. The unique
// index on provider enforces one policy per provider.
func (s *RDBConfigStore) CreateCircuitBreakerConfig(ctx context.Context, config *tables.TableCircuitBreakerConfig, tx ...*gorm.DB) error {
	txDB := s.DB()
	if len(tx) > 0 {
		txDB = tx[0]
	}
...
// UpdateCircuitBreakerConfig saves an existing circuit breaker policy.
func (s *RDBConfigStore) UpdateCircuitBreakerConfig(ctx context.Context, config *tables.TableCircuitBreakerConfig, tx ...*gorm.DB) error {
	txDB := s.DB()
	if len(tx) > 0 {
		txDB = tx[0]
	}
...
// DeleteCircuitBreakerConfig deletes a circuit breaker policy by ID.
func (s *RDBConfigStore) DeleteCircuitBreakerConfig(ctx context.Context, id string, tx ...*gorm.DB) error {
	txDB := s.DB()
	if len(tx) > 0 {
		txDB = tx[0]
	}
...
// GetAlertChannels returns every alert channel, newest first. Follows
// GetCircuitBreakerConfigs.
func (s *RDBConfigStore) GetAlertChannels(ctx context.Context) ([]tables.TableAlertChannel, error) {
...
func (s *RDBConfigStore) CreateAlertChannel(ctx context.Context, channel *tables.TableAlertChannel, tx ...*gorm.DB) error {
	txDB := s.DB()
	if len(tx) > 0 {
		txDB = tx[0]
	}
	if err := txDB.WithContext(ctx).Create(channel).Error; err != nil {
		return s.parseGormError(err)
	}
	return nil
}

// UpdateAlertChannel saves an existing alert channel.
func (s *RDBConfigStore) UpdateAlertChannel(ctx context.Context, channel *tables.TableAlertChannel, tx ...*gorm.DB) error {
	txDB := s.DB()
	if len(tx) > 0 {
		txDB = tx[0]
	}
	if err := txDB.WithContext(ctx).Save(channel).Error; err != nil {
		return s.parseGormError(err)
	}
	return nil
}
...
func (s *RDBConfigStore) DeleteAlertChannel(ctx context.Context, id string, tx ...*gorm.DB) error {
	txDB := s.DB()
	if len(tx) > 0 {
		txDB = tx[0]
	}
	var channel tables.TableAlertChannel
	if err := txDB.WithContext(ctx).Where("id = ?", id).First(&channel).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return ErrNotFound
		}
		return err
	}
	return txDB.WithContext(ctx).Delete(&channel).Error
}

// UpdateAlertChannelDeliveryStatus writes only the last-delivery bookkeeping
// columns for a channel, bypassing the model hooks (no secret re-encryption,
// no filter re-serialization) so the dispatcher's best-effort writeback cannot
// clobber a concurrent admin edit.
func (s *RDBConfigStore) UpdateAlertChannelDeliveryStatus(ctx context.Context, id string, attemptAt time.Time, status string, lastError string) error {
	result := s.DB().WithContext(ctx).
		Model(&tables.TableAlertChannel{}).
		Where("id = ?", id).
...
// ----- JWT auth issuer config CRUD -----

// GetJWTAuthConfigs returns every JWT auth issuer config, newest first.
// Follows GetCircuitBreakerConfigs.
func (s *RDBConfigStore) GetJWTAuthConfigs(ctx context.Context) ([]tables.TableJWTAuthConfig, error) {
	var configs []tables.TableJWTAuthConfig
	if err := s.DB().WithContext(ctx).
		Order("created_at DESC, id DESC").
		Find(&configs).Error; err != nil {
		return nil, err
...
// GetJWTAuthConfig retrieves a single JWT auth issuer config by ID.
func (s *RDBConfigStore) GetJWTAuthConfig(ctx context.Context, id string) (*tables.TableJWTAuthConfig, error) {
	var config tables.TableJWTAuthConfig
	if err := s.DB().WithContext(ctx).First(&config, "id = ?", id).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, ErrNotFound
		}
...
// CreateJWTAuthConfig inserts a new issuer config. The unique index on issuer
// enforces one config per issuer.
func (s *RDBConfigStore) CreateJWTAuthConfig(ctx context.Context, config *tables.TableJWTAuthConfig, tx ...*gorm.DB) error {
	txDB := s.DB()
	if len(tx) > 0 {
		txDB = tx[0]
	}
...
// UpdateJWTAuthConfig saves an existing issuer config.
func (s *RDBConfigStore) UpdateJWTAuthConfig(ctx context.Context, config *tables.TableJWTAuthConfig, tx ...*gorm.DB) error {
	txDB := s.DB()
	if len(tx) > 0 {
		txDB = tx[0]
	}
...
// DeleteJWTAuthConfig deletes an issuer config by ID.
func (s *RDBConfigStore) DeleteJWTAuthConfig(ctx context.Context, id string, tx ...*gorm.DB) error {
	txDB := s.DB()
	if len(tx) > 0 {
		txDB = tx[0]
	}
...
// ----- Prompt deployment CRUD -----

// GetPromptDeployments returns every deployment owned by a prompt, newest first.
// Follows GetGuardrailConfigs.
func (s *RDBConfigStore) GetPromptDeployments(ctx context.Context, promptID string) ([]tables.TablePromptDeployment, error) {
	var deployments []tables.TablePromptDeployment
	if err := s.DB().WithContext(ctx).
...
// GetPromptDeployment retrieves a single prompt deployment by ID.
func (s *RDBConfigStore) GetPromptDeployment(ctx context.Context, id string) (*tables.TablePromptDeployment, error) {
	var deployment tables.TablePromptDeployment
	if err := s.DB().WithContext(ctx).First(&deployment, "id = ?", id).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, ErrNotFound
		}
		return nil, err
...
// GetActivePromptDeployments returns every enabled prompt deployment. Used by
// the prompts plugin's deployment resolver to build its in-memory routing table.
// Follows GetEnabledGuardrailConfigs.
func (s *RDBConfigStore) GetActivePromptDeployments(ctx context.Context) ([]tables.TablePromptDeployment, error) {
	var deployments []tables.TablePromptDeployment
	if err := s.DB().WithContext(ctx).
		Where("enabled = ?", true).
		Order("created_at ASC, id ASC").
...
// CreatePromptDeployment inserts a new prompt deployment.
func (s *RDBConfigStore) CreatePromptDeployment(ctx context.Context, deployment *tables.TablePromptDeployment, tx ...*gorm.DB) error {
	txDB := s.DB()
	if len(tx) > 0 {
		txDB = tx[0]
	}
	if err := txDB.WithContext(ctx).Create(deployment).Error; err != nil {
		return s.parseGormError(err)
	}
	return nil
}

// UpdatePromptDeployment saves an existing prompt deployment (full overwrite of
// mutable fields). The caller is expected to have loaded the row first.
func (s *RDBConfigStore) UpdatePromptDeployment(ctx context.Context, deployment *tables.TablePromptDeployment, tx ...*gorm.DB) error {
	txDB := s.DB()
	if len(tx) > 0 {
		txDB = tx[0]
	}
	if err := txDB.WithContext(ctx).Save(deployment).Error; err != nil {
		return s.parseGormError(err)
	}
	return nil
}

// DeletePromptDeployment deletes a prompt deployment by ID.
func (s *RDBConfigStore) DeletePromptDeployment(ctx context.Context, id string, tx ...*gorm.DB) error {
	txDB := s.DB()
	if len(tx) > 0 {
		txDB = tx[0]
	}
	var deployment tables.TablePromptDeployment
	if err := txDB.WithContext(ctx).Where("id = ?", id).First(&deployment).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return ErrNotFound
		}
		return err
	}
	return txDB.WithContext(ctx).Delete(&deployment).Error
}

// ----- MCP tool group CRUD -----

// GetMCPToolGroups returns a filtered, paginated page of MCP tool groups plus
// the total matching count. Reads through ScopedDB so any QueryScope on ctx
// (installed by the upstream MCP visibility middleware for EntityMCPToolGroup)
// narrows the result to groups the caller is allowed to see. Follows
// GetGuardrailConfigs.
func (s *RDBConfigStore) GetMCPToolGroups(ctx context.Context, params MCPToolGroupsQueryParams) ([]tables.TableMCPToolGroup, int64, error) {
	baseQuery := s.ScopedDB(ctx).Model(&tables.TableMCPToolGroup{})
...
// GetMCPToolGroup retrieves a single MCP tool group by ID. Reads through
// ScopedDB so a group the caller may not see returns ErrNotFound, matching the
// leak-prevention contract used by the other governance entities.
func (s *RDBConfigStore) GetMCPToolGroup(ctx context.Context, id string) (*tables.TableMCPToolGroup, error) {
	var group tables.TableMCPToolGroup
	if err := s.ScopedDB(ctx).First(&group, "id = ?", id).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, ErrNotFound
		}
		return nil, err
...
func (s *RDBConfigStore) CreateMCPToolGroup(ctx context.Context, group *tables.TableMCPToolGroup, tx ...*gorm.DB) error {
	txDB := s.DB()
	if len(tx) > 0 {
		txDB = tx[0]
	}
	if err := txDB.WithContext(ctx).Create(group).Error; err != nil {
		return s.parseGormError(err)
	}
	return nil
}

// UpdateMCPToolGroup saves an existing MCP tool group (full overwrite of mutable
// fields). The caller is expected to have loaded the row first.
func (s *RDBConfigStore) UpdateMCPToolGroup(ctx context.Context, group *tables.TableMCPToolGroup, tx ...*gorm.DB) error {
	txDB := s.DB()
	if len(tx) > 0 {
		txDB = tx[0]
	}
	if err := txDB.WithContext(ctx).Save(group).Error; err != nil {
		return s.parseGormError(err)
	}
	return nil
}

// DeleteMCPToolGroup deletes an MCP tool group by ID.
func (s *RDBConfigStore) DeleteMCPToolGroup(ctx context.Context, id string, tx ...*gorm.DB) error {
	txDB := s.DB()
	if len(tx) > 0 {
		txDB = tx[0]
	}
	var group tables.TableMCPToolGroup
	if err := txDB.WithContext(ctx).Where("id = ?", id).First(&group).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return ErrNotFound
		}
		return err
	}
	return txDB.WithContext(ctx).Delete(&group).Error
}

// ----- Audit log append/query -----

// applyAuditLogFilters applies the AuditLogsQueryParams filter fields (not
// Limit/Offset) to an audit-log query. Shared by GetAuditLogs (offset
// pagination) and GetAuditLogsSince (keyset pagination for export/verify).
func applyAuditLogFilters(query *gorm.DB, params AuditLogsQueryParams) *gorm.DB {
	if params.Action != "" {
	...
	if params.Actor != "" {
		query = query.Where("LOWER(actor) LIKE ?", "%"+strings.ToLower(params.Actor)+"%")
	}
	if params.Target != "" {
	...
	if params.Search != "" {
		search := "%" + strings.ToLower(params.Search) + "%"
		query = query.Where(
			"LOWER(action) LIKE ? OR LOWER(actor) LIKE ? OR LOWER(target) LIKE ?",
			search, search, search,
		)
	}
	if params.Since != nil {
		query = query.Where("timestamp >= ?", *params.Since)
	}
	if params.Until != nil {
		query = query.Where("timestamp <= ?", *params.Until)
	}
	return query
}

// GetAuditLogs returns a filtered, paginated page of audit log rows plus the
// total matching count, newest first. Follows GetGuardrailConfigs.
func (s *RDBConfigStore) GetAuditLogs(ctx context.Context, params AuditLogsQueryParams) ([]tables.TableAuditLog, int64, error) {
	baseQuery := applyAuditLogFilters(s.DB().WithContext(ctx).Model(&tables.TableAuditLog{}), params)
...
// CreateAuditLog appends a single audit log row. The audit log is append-only,
// so there is no corresponding update or delete.
func (s *RDBConfigStore) CreateAuditLog(ctx context.Context, log *tables.TableAuditLog, tx ...*gorm.DB) error {
	txDB := s.DB()
	if len(tx) > 0 {
		txDB = tx[0]
	}
	if err := txDB.WithContext(ctx).Create(log).Error; err != nil {
		return s.parseGormError(err)
	}
	return nil
}

// GetAuditLogSettings returns the singleton audit-log retention/export settings
// row, or ErrNotFound when the feature has never been configured (the
// default-off state).
func (s *RDBConfigStore) GetAuditLogSettings(ctx context.Context) (*tables.TableAuditLogSettings, error) {
	var settings tables.TableAuditLogSettings
	if err := s.DB().WithContext(ctx).Where("id = ?", tables.AuditLogSettingsID).First(&settings).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, ErrNotFound
		}
		return nil, err
...
// UpdateAuditLogSettings upserts the singleton audit-log settings row. The
// fixed primary key (pinned in BeforeSave) guarantees at most one row exists.
func (s *RDBConfigStore) UpdateAuditLogSettings(ctx context.Context, settings *tables.TableAuditLogSettings) error {
	if settings == nil {
		return fmt.Errorf("audit log settings cannot be nil")
	}
	settings.ID = tables.AuditLogSettingsID
...
// auditLogPruneRef is the projection used by the retention delete helpers: the
// row IDs to delete plus their signatures, so the caller can anchor the
// deletion in a signed prune-marker event.
type auditLogPruneRef struct {
	ID        string
	Signature string
}

// deleteAuditLogRefs deletes the referenced audit rows and returns the count
// plus their signatures (in the oldest-first order they were selected).
func (s *RDBConfigStore) deleteAuditLogRefs(ctx context.Context, refs []auditLogPruneRef) (int64, []string, error) {
	if len(refs) == 0 {
		return 0, nil, nil
	}
	ids := make([]string, len(refs))
	signatures := make([]string, len(refs))
	for i, ref := range refs {
		ids[i] = ref.ID
		signatures[i] = ref.Signature
	}
	result := s.DB().WithContext(ctx).Where("id IN ?", ids).Delete(&tables.TableAuditLog{})
	if result.Error != nil {
		return 0, nil, s.parseGormError(result.Error)
	}
	return result.RowsAffected, signatures, nil
}

// DeleteAuditLogsBefore deletes at most batchSize audit rows with a timestamp
// strictly before cutoff, oldest-first by (timestamp, id). It returns the
// deleted count and the deleted rows' signatures; the retention worker calls it
// in a loop until it returns 0. Deleting oldest-first guarantees the newest
// rows — including any previously written prune-marker events — are the last
// ever considered for deletion.
func (s *RDBConfigStore) DeleteAuditLogsBefore(ctx context.Context, cutoff time.Time, batchSize int) (int64, []string, error) {
	if batchSize <= 0 {
		batchSize = 100
	}
	var refs []auditLogPruneRef
	if err := s.DB().WithContext(ctx).
		Model(&tables.TableAuditLog{}).
	...
	return s.deleteAuditLogRefs(ctx, refs)
}

// TrimAuditLogsToCount deletes at most batchSize of the oldest audit rows (by
// timestamp, id) so long as the table holds more than maxRows rows. It returns
// the deleted count and the deleted rows' signatures; the retention worker
// calls it in a loop until it returns 0. Always deletes oldest-first so the
// newest rows (e.g. fresh prune-marker events) are never trimmed before older
// evidence.
func (s *RDBConfigStore) TrimAuditLogsToCount(ctx context.Context, maxRows int64, batchSize int) (int64, []string, error) {
	if maxRows <= 0 {
		return 0, nil, nil
	}
	if batchSize <= 0 {
	...
	var total int64
	if err := s.DB().WithContext(ctx).Model(&tables.TableAuditLog{}).Count(&total).Error; err != nil {
		return 0, nil, err
	}
...
// GetAuditLogsSince returns up to limit audit rows strictly after the
// (afterTimestamp, afterID) keyset cursor, ordered (timestamp ASC, id ASC), and
// filtered by params (Limit/Offset in params are ignored). Keyset pagination
// keeps the NDJSON export/verify iteration stable and index-friendly on large
// tables where OFFSET would degrade.
func (s *RDBConfigStore) GetAuditLogsSince(ctx context.Context, params AuditLogsQueryParams, afterTimestamp time.Time, afterID string, limit int) ([]tables.TableAuditLog, error) {
	if limit <= 0 {
		limit = 500
	}
	query := applyAuditLogFilters(s.DB().WithContext(ctx).Model(&tables.TableAuditLog{}), params)
...
// ----- User & business unit CRUD -----

// GetUsers returns a filtered, paginated page of users plus the total matching
// count. Budget, rate-limit and business-unit relationships are preloaded for
// read-side hydration. Follows GetGuardrailConfigs.
func (s *RDBConfigStore) GetUsers(ctx context.Context, params UsersQueryParams) ([]tables.TableUser, int64, error) {
	...
	if params.BusinessUnitID != "" {
		baseQuery = baseQuery.Where("business_unit_id = ?", params.BusinessUnitID)
	}
...
// GetUser retrieves a single user by ID with relationships preloaded.
...
// UpdateUser saves an existing user (full overwrite of mutable fields). The
// caller is expected to have loaded the row first.
...
func (s *RDBConfigStore) DeleteUser(ctx context.Context, id string, tx ...*gorm.DB) error {
	txDB := s.DB()
	if len(tx) > 0 {
		txDB = tx[0]
	}
	var user tables.TableUser
	if err := txDB.WithContext(ctx).Where("id = ?", id).First(&user).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return ErrNotFound
		}
		return err
	}
	return txDB.WithContext(ctx).Delete(&user).Error
}

// GetBusinessUnits returns a filtered, paginated page of business units plus the
// total matching count. The transient UserCount is populated via a correlated
// subquery, and budget/rate-limit relationships are preloaded.
func (s *RDBConfigStore) GetBusinessUnits(ctx context.Context, params BusinessUnitsQueryParams) ([]tables.TableBusinessUnit, int64, error) {
	baseQuery := s.DB().WithContext(ctx).Model(&tables.TableBusinessUnit{})
	...
	if err := baseQuery.
		Select("governance_business_units.*, (SELECT COUNT(*) FROM governance_users WHERE governance_users.business_unit_id = governance_business_units.id) AS user_count").
		Preload("Budget").
		Preload("RateLimit").
		Order("created_at DESC, id DESC").
		Offset(offset).
...
// GetBusinessUnit retrieves a single business unit by ID with relationships
// preloaded.
...
// CreateBusinessUnit inserts a new business unit.
func (s *RDBConfigStore) CreateBusinessUnit(ctx context.Context, bu *tables.TableBusinessUnit, tx ...*gorm.DB) error {
	txDB := s.DB()
	if len(tx) > 0 {
		txDB = tx[0]
	}
	if err := txDB.WithContext(ctx).Create(bu).Error; err != nil {
		return s.parseGormError(err)
	}
	return nil
}

// UpdateBusinessUnit saves an existing business unit (full overwrite of mutable
// fields). The caller is expected to have loaded the row first.
func (s *RDBConfigStore) UpdateBusinessUnit(ctx context.Context, bu *tables.TableBusinessUnit, tx ...*gorm.DB) error {
	txDB := s.DB()
	if len(tx) > 0 {
		txDB = tx[0]
	}
	if err := txDB.WithContext(ctx).Save(bu).Error; err != nil {
		return s.parseGormError(err)
	}
	return nil
}

// DeleteBusinessUnit deletes a business unit by ID. Users referencing it are
// detached (business_unit_id set to NULL) so no orphaned FK remains.
func (s *RDBConfigStore) DeleteBusinessUnit(ctx context.Context, id string, tx ...*gorm.DB) error {
	txDB := s.DB()
	if len(tx) > 0 {
		txDB = tx[0]
	}
	var unit tables.TableBusinessUnit
	if err := txDB.WithContext(ctx).Where("id = ?", id).First(&unit).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return ErrNotFound
		}
		return err
	...
	if err := txDB.WithContext(ctx).
		...
		Update("business_unit_id", nil).Error; err != nil {
		return err
	}
	return txDB.WithContext(ctx).Delete(&unit).Error
}

// ----- RBAC role / permission / assignment CRUD -----

// GetRoles returns a filtered, paginated page of roles plus the total matching
// count, with each role's Permissions preloaded. Follows GetGuardrailConfigs.
func (s *RDBConfigStore) GetRoles(ctx context.Context, params RolesQueryParams) ([]tables.TableRole, int64, error) {
	baseQuery := s.DB().WithContext(ctx).Model(&tables.TableRole{})
	...
	if err := baseQuery.
		Preload("Permissions").
		Order("is_system DESC, created_at DESC, id DESC").
		Offset(offset).
		Limit(limit).
...
// GetRole retrieves a single role by ID with Permissions preloaded.
...
// GetRoleByName retrieves a single role by name with Permissions preloaded.
func (s *RDBConfigStore) GetRoleByName(ctx context.Context, name string) (*tables.TableRole, error) {
	var role tables.TableRole
	if err := s.DB().WithContext(ctx).
		Preload("Permissions").
...
// CreateRole inserts a new role. Permissions on the struct are persisted by GORM
// association upserts.
func (s *RDBConfigStore) CreateRole(ctx context.Context, role *tables.TableRole, tx ...*gorm.DB) error {
	...
	if err := txDB.WithContext(ctx).Create(role).Error; err != nil {
		return s.parseGormError(err)
	}
	return nil
}

// UpdateRole saves the mutable fields of an existing role (name, description).
// Permission membership is managed separately via ReplaceRolePermissions so an
// edit does not silently drop or duplicate grants.
...
// DeleteRole deletes a role by ID. Its permissions and assignments are removed
// explicitly (rather than relying on DB cascade) so the behavior is identical on
// SQLite and Postgres.
func (s *RDBConfigStore) DeleteRole(ctx context.Context, id string, tx ...*gorm.DB) error {
	...
	var role tables.TableRole
	if err := txDB.WithContext(ctx).Where("id = ?", id).First(&role).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return ErrNotFound
		}
		return err
	}
	if err := txDB.WithContext(ctx).Where("role_id = ?", id).Delete(&tables.TablePermission{}).Error; err != nil {
		return err
	}
	if err := txDB.WithContext(ctx).Where("role_id = ?", id).Delete(&tables.TableRoleAssignment{}).Error; err != nil {
		return err
	}
	return txDB.WithContext(ctx).Delete(&role).Error
}

// ReplaceRolePermissions atomically replaces all permissions of a role with the
// supplied set: it deletes the existing grants and inserts the new ones. The
// caller assigns IDs/RoleID on each permission.
func (s *RDBConfigStore) ReplaceRolePermissions(ctx context.Context, roleID string, perms []tables.TablePermission, tx ...*gorm.DB) error {
	txDB := s.DB()
	if len(tx) > 0 {
		txDB = tx[0]
	}
	if err := txDB.WithContext(ctx).Where("role_id = ?", roleID).Delete(&tables.TablePermission{}).Error; err != nil {
		return err
	}
	if len(perms) == 0 {
		return nil
	}
	if err := txDB.WithContext(ctx).Create(&perms).Error; err != nil {
		return s.parseGormError(err)
	}
	return nil
}

// GetRoleAssignments returns a filtered, paginated page of role assignments plus
// the total matching count, with Role (and its Permissions) and User preloaded.
func (s *RDBConfigStore) GetRoleAssignments(ctx context.Context, params RoleAssignmentsQueryParams) ([]tables.TableRoleAssignment, int64, error) {
	baseQuery := s.DB().WithContext(ctx).Model(&tables.TableRoleAssignment{})

	if params.UserID != "" {
		baseQuery = baseQuery.Where("user_id = ?", params.UserID)
	}
	if params.RoleID != "" {
		baseQuery = baseQuery.Where("role_id = ?", params.RoleID)
	}
...
// GetRoleAssignment retrieves a single role assignment by ID with Role and User
// preloaded.
func (s *RDBConfigStore) GetRoleAssignment(ctx context.Context, id string) (*tables.TableRoleAssignment, error) {
	var assignment tables.TableRoleAssignment
	if err := s.DB().WithContext(ctx).
		Preload("Role").
		Preload("Role.Permissions").
		Preload("User").
...
// GetRoleAssignmentsByUser returns every role assignment for a user with each
// assignment's Role and that role's Permissions preloaded. Used by the RBAC
// middleware to resolve a user's effective permissions.
func (s *RDBConfigStore) GetRoleAssignmentsByUser(ctx context.Context, userID string) ([]tables.TableRoleAssignment, error) {
	var assignments []tables.TableRoleAssignment
	if err := s.DB().WithContext(ctx).
		Preload("Role").
		Preload("Role.Permissions").
...
// CreateRoleAssignment inserts a new role assignment.
func (s *RDBConfigStore) CreateRoleAssignment(ctx context.Context, assignment *tables.TableRoleAssignment, tx ...*gorm.DB) error {
	txDB := s.DB()
	if len(tx) > 0 {
		txDB = tx[0]
	}
	if err := txDB.WithContext(ctx).Create(assignment).Error; err != nil {
		return s.parseGormError(err)
	}
	return nil
}

// DeleteRoleAssignment deletes a role assignment by ID.
func (s *RDBConfigStore) DeleteRoleAssignment(ctx context.Context, id string, tx ...*gorm.DB) error {
	txDB := s.DB()
	if len(tx) > 0 {
		txDB = tx[0]
	}
	var assignment tables.TableRoleAssignment
	if err := txDB.WithContext(ctx).Where("id = ?", id).First(&assignment).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return ErrNotFound
		}
		return err
	}
	return txDB.WithContext(ctx).Delete(&assignment).Error
}

// CountRoleAssignments returns the total number of role assignments. The RBAC
// middleware uses this to stay permissive (fail-open) until at least one
// assignment exists, so enabling RBAC on an unconfigured deployment never bricks
// access.
func (s *RDBConfigStore) CountRoleAssignments(ctx context.Context) (int64, error) {
	var count int64
	if err := s.DB().WithContext(ctx).Model(&tables.TableRoleAssignment{}).Count(&count).Error; err != nil {
		return 0, err
	}
	return count, nil
}
...
// GetAPIKeys returns a filtered, paginated page of admin API keys plus the total
// matching count, with each key's Scopes preloaded. Follows GetRoles.
func (s *RDBConfigStore) GetAPIKeys(ctx context.Context, params APIKeysQueryParams) ([]tables.TableAdminAPIKey, int64, error) {
	baseQuery := s.DB().WithContext(ctx).Model(&tables.TableAdminAPIKey{})
	...
	var keys []tables.TableAdminAPIKey
	if err := baseQuery.
		Preload("Scopes").
		Order("created_at DESC, id DESC").
		Offset(offset).
...
// GetAPIKey retrieves a single admin API key by ID with Scopes preloaded.
func (s *RDBConfigStore) GetAPIKey(ctx context.Context, id string) (*tables.TableAdminAPIKey, error) {
	var key tables.TableAdminAPIKey
	if err := s.DB().WithContext(ctx).
		Preload("Scopes").
...
// GetAPIKeyByValueHash retrieves a single admin API key by the SHA-256 hash of
// its plaintext value, with Scopes preloaded. This is the API key middleware's
// lookup path (unique value_hash index).
func (s *RDBConfigStore) GetAPIKeyByValueHash(ctx context.Context, valueHash string) (*tables.TableAdminAPIKey, error) {
	var key tables.TableAdminAPIKey
	if err := s.DB().WithContext(ctx).
		Preload("Scopes").
		First(&key, "value_hash = ?", valueHash).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, ErrNotFound
		}
		return nil, err
...
// CreateAPIKey inserts a new admin API key. Scope membership is managed
// separately via ReplaceAPIKeyScopes so association writes stay explicit.
func (s *RDBConfigStore) CreateAPIKey(ctx context.Context, key *tables.TableAdminAPIKey, tx ...*gorm.DB) error {
	txDB := s.DB()
	if len(tx) > 0 {
		txDB = tx[0]
	}
	if err := txDB.WithContext(ctx).Omit(clause.Associations).Create(key).Error; err != nil {
		return s.parseGormError(err)
	}
	return nil
}

// UpdateAPIKey saves the mutable fields of an existing admin API key (name,
// status, expiry, rotation lineage and — via the transient Value/BeforeSave hash
// path — a rotated secret). Scope membership is managed separately via
// ReplaceAPIKeyScopes; associations are omitted so an update never silently
// rewrites grants.
func (s *RDBConfigStore) UpdateAPIKey(ctx context.Context, key *tables.TableAdminAPIKey, tx ...*gorm.DB) error {
	txDB := s.DB()
	if len(tx) > 0 {
		txDB = tx[0]
	}
	if err := txDB.WithContext(ctx).Omit(clause.Associations).Save(key).Error; err != nil {
		return s.parseGormError(err)
	}
	return nil
}

// DeleteAPIKey deletes an admin API key by ID. Its scopes are removed explicitly
// (rather than relying on DB cascade) so the behavior is identical on SQLite and
// Postgres. Follows DeleteRole.
func (s *RDBConfigStore) DeleteAPIKey(ctx context.Context, id string, tx ...*gorm.DB) error {
	txDB := s.DB()
	if len(tx) > 0 {
		txDB = tx[0]
	}
	var key tables.TableAdminAPIKey
	if err := txDB.WithContext(ctx).Where("id = ?", id).First(&key).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return ErrNotFound
		}
		return err
	}
	if err := txDB.WithContext(ctx).Where("api_key_id = ?", id).Delete(&tables.TableAdminAPIKeyScope{}).Error; err != nil {
		return err
	}
	return txDB.WithContext(ctx).Delete(&key).Error
}

// ReplaceAPIKeyScopes atomically replaces all scopes of an admin API key with
// the supplied set: it deletes the existing grants and inserts the new ones.
// The caller assigns IDs/APIKeyID on each scope. Follows ReplaceRolePermissions.
func (s *RDBConfigStore) ReplaceAPIKeyScopes(ctx context.Context, keyID string, scopes []tables.TableAdminAPIKeyScope, tx ...*gorm.DB) error {
	txDB := s.DB()
	if len(tx) > 0 {
		txDB = tx[0]
	}
	if err := txDB.WithContext(ctx).Where("api_key_id = ?", keyID).Delete(&tables.TableAdminAPIKeyScope{}).Error; err != nil {
		return err
	}
	if len(scopes) == 0 {
		return nil
	}
	if err := txDB.WithContext(ctx).Create(&scopes).Error; err != nil {
		return s.parseGormError(err)
	}
	return nil
}

// TouchAPIKeyLastUsed stamps a key's last_used_at. Called asynchronously (and
// throttled) by the API key middleware; a failure here must never affect the
// authenticated request. UpdateColumn skips hooks and updated_at on purpose —
// a usage stamp is not an edit.
func (s *RDBConfigStore) TouchAPIKeyLastUsed(ctx context.Context, id string, usedAt time.Time) error {
	return s.DB().WithContext(ctx).
		Model(&tables.TableAdminAPIKey{}).
		Where("id = ?", id).
...
// GetLargePayloadConfig retrieves the large-payload streaming configuration from the database.
// Returns nil when no configuration has been persisted yet.
func (s *RDBConfigStore) GetLargePayloadConfig(ctx context.Context) (*tables.LargePayloadConfig, error) {
	var configEntry tables.TableGovernanceConfig
	if err := s.DB().WithContext(ctx).First(&configEntry, "key = ?", tables.ConfigLargePayloadKey).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
		return nil, err
	...
	var lpConfig tables.LargePayloadConfig
	if err := json.Unmarshal([]byte(configEntry.Value), &lpConfig); err != nil {
		return nil, fmt.Errorf("failed to unmarshal large payload config: %w", err)
	}
	return &lpConfig, nil
}

// UpdateLargePayloadConfig persists the large-payload streaming configuration in the database.
func (s *RDBConfigStore) UpdateLargePayloadConfig(ctx context.Context, config *tables.LargePayloadConfig) error {
	configJSON, err := json.Marshal(config)
	if err != nil {
		return fmt.Errorf("failed to marshal large payload config: %w", err)
	}
	return s.DB().WithContext(ctx).Save(&tables.TableGovernanceConfig{
		Key:   tables.ConfigLargePayloadKey,
		Value: string(configJSON),
	}).Error
}
...
// ---- SCIM provisioning CRUD (SSO/SCIM sync engine — Keycloak in this slice) ----

// GetSCIMUsers returns a filtered, paginated page of synced SCIM users plus the
// total matching count, newest first. Follows GetUsers.
func (s *RDBConfigStore) GetSCIMUsers(ctx context.Context, params SCIMUsersQueryParams) ([]tables.TableSCIMUser, int64, error) {
	baseQuery := s.DB().WithContext(ctx).Model(&tables.TableSCIMUser{})
	if params.Search != "" {
		search := "%" + strings.ToLower(params.Search) + "%"
		baseQuery = baseQuery.Where("LOWER(user_name) LIKE ? OR LOWER(email) LIKE ? OR LOWER(display_name) LIKE ?", search, search, search)
	}
	if params.Provider != "" {
		baseQuery = baseQuery.Where("provider = ?", strings.ToLower(params.Provider))
	}
	if params.Active != nil {
		baseQuery = baseQuery.Where("active = ?", *params.Active)
	}
	...
	var users []tables.TableSCIMUser
	if err := baseQuery.
		Order("created_at DESC, id DESC").
		Offset(offset).
		Limit(limit).
...
// GetSCIMUser retrieves a single synced SCIM user by internal ID.
func (s *RDBConfigStore) GetSCIMUser(ctx context.Context, id string) (*tables.TableSCIMUser, error) {
	var user tables.TableSCIMUser
...
// GetSCIMUserByExternalID retrieves a synced SCIM user by its IdP-side identity.
// Returns ErrNotFound when no row matches.
func (s *RDBConfigStore) GetSCIMUserByExternalID(ctx context.Context, provider, externalID string) (*tables.TableSCIMUser, error) {
	var user tables.TableSCIMUser
	if err := s.DB().WithContext(ctx).
		First(&user, "provider = ? AND external_id = ?", strings.ToLower(provider), externalID).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, ErrNotFound
		}
		return nil, err
...
// UpsertSCIMUser inserts or updates a synced SCIM user, keyed on
// (provider, external_id). It is idempotent so a re-run of the sync engine
// converges on the same row rather than creating duplicates.
func (s *RDBConfigStore) UpsertSCIMUser(ctx context.Context, user *tables.TableSCIMUser, tx ...*gorm.DB) error {
	txDB := s.DB()
	if len(tx) > 0 {
		txDB = tx[0]
	}
	existing, err := s.GetSCIMUserByExternalID(ctx, user.Provider, user.ExternalID)
	if err != nil && !errors.Is(err, ErrNotFound) {
		return err
	}
	if existing != nil {
		// Preserve the stable internal ID and creation timestamp.
		user.ID = existing.ID
		user.CreatedAt = existing.CreatedAt
		// Don't clobber an already-resolved managed-user link with nil.
		if user.ManagedUserID == nil {
			user.ManagedUserID = existing.ManagedUserID
		}
		if err := txDB.WithContext(ctx).Save(user).Error; err != nil {
			return s.parseGormError(err)
		}
...
// DeleteSCIMUser hard-deletes a synced SCIM user row by internal ID. The sync
// engine prefers deactivation (Active=false) over deletion; this exists for the
// management UI and full re-sync cleanup.
func (s *RDBConfigStore) DeleteSCIMUser(ctx context.Context, id string, tx ...*gorm.DB) error {
	txDB := s.DB()
	if len(tx) > 0 {
		txDB = tx[0]
	}
...
// GetSCIMGroups returns a filtered, paginated page of synced SCIM groups plus
// the total matching count, newest first. Follows GetSCIMUsers.
func (s *RDBConfigStore) GetSCIMGroups(ctx context.Context, params SCIMGroupsQueryParams) ([]tables.TableSCIMGroup, int64, error) {
	baseQuery := s.DB().WithContext(ctx).Model(&tables.TableSCIMGroup{})
	if params.Search != "" {
		search := "%" + strings.ToLower(params.Search) + "%"
		baseQuery = baseQuery.Where("LOWER(display_name) LIKE ?", search)
	}
	if params.Provider != "" {
		baseQuery = baseQuery.Where("provider = ?", strings.ToLower(params.Provider))
	}
	...
	var groups []tables.TableSCIMGroup
	if err := baseQuery.
		Order("created_at DESC, id DESC").
		Offset(offset).
		Limit(limit).
...
// GetSCIMGroupByExternalID retrieves a synced SCIM group by its IdP-side ID.
func (s *RDBConfigStore) GetSCIMGroupByExternalID(ctx context.Context, provider, externalID string) (*tables.TableSCIMGroup, error) {
	var group tables.TableSCIMGroup
	if err := s.DB().WithContext(ctx).
		First(&group, "provider = ? AND external_id = ?", strings.ToLower(provider), externalID).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, ErrNotFound
		}
		return nil, err
...
// UpsertSCIMGroup inserts or updates a synced SCIM group, keyed on
// (provider, external_id). Idempotent, mirroring UpsertSCIMUser.
func (s *RDBConfigStore) UpsertSCIMGroup(ctx context.Context, group *tables.TableSCIMGroup, tx ...*gorm.DB) error {
	txDB := s.DB()
	if len(tx) > 0 {
		txDB = tx[0]
	}
	existing, err := s.GetSCIMGroupByExternalID(ctx, group.Provider, group.ExternalID)
	if err != nil && !errors.Is(err, ErrNotFound) {
		return err
	}
	if existing != nil {
...
// DeleteSCIMGroup hard-deletes a synced SCIM group row by internal ID.
func (s *RDBConfigStore) DeleteSCIMGroup(ctx context.Context, id string, tx ...*gorm.DB) error {
	txDB := s.DB()
	if len(tx) > 0 {
		txDB = tx[0]
	}
