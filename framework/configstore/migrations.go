...
var configstoreMigrationSteps = []migrationStep{
	...
	{IDs: []string{"add_guardrail_config_table"}, run: migrationAddGuardrailConfigTable},
	{IDs: []string{"add_pii_rule_table"}, run: migrationAddPIIRuleTable},
	{IDs: []string{"add_business_unit_table"}, run: migrationAddBusinessUnitTable},
	{IDs: []string{"add_user_table"}, run: migrationAddUserTable},
	{IDs: []string{"add_rbac_tables"}, run: migrationAddRBACTables},
	{IDs: []string{"add_audit_log_table"}, run: migrationAddAuditLogTable},
	{IDs: []string{"add_mcp_tool_group_table"}, run: migrationAddMCPToolGroupTable},
	{IDs: []string{"add_prompt_deployment_table"}, run: migrationAddPromptDeploymentTable},
	{IDs: []string{"add_circuit_breaker_config_table"}, run: migrationAddCircuitBreakerConfigTable},
	{IDs: []string{"add_scim_tables"}, run: migrationAddSCIMTables},
	{IDs: []string{"add_admin_api_key_tables"}, run: migrationAddAdminAPIKeyTables},
	{IDs: []string{"add_audit_log_settings_table"}, run: migrationAddAuditLogSettingsTable},
	{IDs: []string{"add_alert_channels_table"}, run: migrationAddAlertChannelsTable},
	{IDs: []string{"add_jwt_auth_config_table"}, run: migrationAddJWTAuthConfigTable},
	{IDs: []string{"add_scim_user_mapping_columns"}, run: migrationAddSCIMUserMappingColumns},
}
...
// migrationAddGuardrailConfigTable adds the governance_guardrail_configs table
// backing the loopback-guard guardrails configuration UI. Follows the
// add_model_config_table migration pattern.
func migrationAddGuardrailConfigTable(ctx context.Context, db *gorm.DB, logger schemas.Logger) error {
	migrationName := "add_guardrail_config_table"
	logger.Info("[configstore] starting migration %s", migrationName)
	defer logger.Info("[configstore] finished migration %s", migrationName)
	m := migrator.New(db, migrator.DefaultOptions, []*migrator.Migration{{
		ID: migrationName,
		Migrate: func(tx *gorm.DB) error {
			tx = tx.WithContext(ctx)
			migrator := tx.Migrator()
			if !migrator.HasTable(&tables.TableGuardrailConfig{}) {
				logger.Info("[configstore] %s: creating table TableGuardrailConfig", migrationName)
				if err := migrator.CreateTable(&tables.TableGuardrailConfig{}); err != nil {
					return err
				}
			}
			return nil
		...
		Rollback: func(tx *gorm.DB) error {
			tx = tx.WithContext(ctx)
			migrator := tx.Migrator()
			logger.Info("[configstore] %s: dropping table TableGuardrailConfig", migrationName)
			if err := migrator.DropTable(&tables.TableGuardrailConfig{}); err != nil {
				return err
			}
			return nil
		},
	...
	if err := m.Migrate(); err != nil {
		return fmt.Errorf("error while running add guardrail config table migration: %s", err.Error())
	}
	return nil
}

// migrationAddMCPToolGroupTable adds the governance_mcp_tool_groups table
// backing the MCP tool groups governance UI. A tool group bundles MCP tool
// references and binds them to a virtual key or team so MCP visibility
// filtering can include/exclude the whole group at once. Follows the
// add_guardrail_config_table migration pattern.
func migrationAddMCPToolGroupTable(ctx context.Context, db *gorm.DB, logger schemas.Logger) error {
	migrationName := "add_mcp_tool_group_table"
	logger.Info("[configstore] starting migration %s", migrationName)
	defer logger.Info("[configstore] finished migration %s", migrationName)
	m := migrator.New(db, migrator.DefaultOptions, []*migrator.Migration{{
		ID: migrationName,
		Migrate: func(tx *gorm.DB) error {
			tx = tx.WithContext(ctx)
			migrator := tx.Migrator()
			if !migrator.HasTable(&tables.TableMCPToolGroup{}) {
				logger.Info("[configstore] %s: creating table TableMCPToolGroup", migrationName)
				if err := migrator.CreateTable(&tables.TableMCPToolGroup{}); err != nil {
					return err
				}
			}
			return nil
		...
		Rollback: func(tx *gorm.DB) error {
			tx = tx.WithContext(ctx)
			migrator := tx.Migrator()
			logger.Info("[configstore] %s: dropping table TableMCPToolGroup", migrationName)
			if err := migrator.DropTable(&tables.TableMCPToolGroup{}); err != nil {
				return err
			}
			return nil
		},
	...
	if err := m.Migrate(); err != nil {
		return fmt.Errorf("error while running add mcp tool group table migration: %s", err.Error())
	}
	return nil
}

// migrationAddPromptDeploymentTable adds the prompt_deployments table backing the
// prompt deployments UI. A deployment is a named, weighted traffic-routing
// strategy for a single prompt: it maps to a set of weighted version refs the
// prompts plugin uses to split live requests across versions (with a fallback to
// the latest version when a pinned version is deleted). Follows the
// add_guardrail_config_table migration pattern.
func migrationAddPromptDeploymentTable(ctx context.Context, db *gorm.DB, logger schemas.Logger) error {
	migrationName := "add_prompt_deployment_table"
	logger.Info("[configstore] starting migration %s", migrationName)
	defer logger.Info("[configstore] finished migration %s", migrationName)
	m := migrator.New(db, migrator.DefaultOptions, []*migrator.Migration{{
		ID: migrationName,
		Migrate: func(tx *gorm.DB) error {
			tx = tx.WithContext(ctx)
			migrator := tx.Migrator()
			if !migrator.HasTable(&tables.TablePromptDeployment{}) {
				logger.Info("[configstore] %s: creating table TablePromptDeployment", migrationName)
				if err := migrator.CreateTable(&tables.TablePromptDeployment{}); err != nil {
					return err
				}
			}
			return nil
		...
		Rollback: func(tx *gorm.DB) error {
			tx = tx.WithContext(ctx)
			migrator := tx.Migrator()
			logger.Info("[configstore] %s: dropping table TablePromptDeployment", migrationName)
			if err := migrator.DropTable(&tables.TablePromptDeployment{}); err != nil {
				return err
			}
			return nil
		},
	...
	if err := m.Migrate(); err != nil {
		return fmt.Errorf("error while running add prompt deployment table migration: %s", err.Error())
	}
	return nil
}

// migrationAddCircuitBreakerConfigTable adds the
// governance_circuit_breaker_configs table backing the per-provider
// circuit-breaker UI. Follows the add_prompt_deployment_table pattern.
func migrationAddCircuitBreakerConfigTable(ctx context.Context, db *gorm.DB, logger schemas.Logger) error {
	migrationName := "add_circuit_breaker_config_table"
	logger.Info("[configstore] starting migration %s", migrationName)
	defer logger.Info("[configstore] finished migration %s", migrationName)
	m := migrator.New(db, migrator.DefaultOptions, []*migrator.Migration{{
		ID: migrationName,
		Migrate: func(tx *gorm.DB) error {
			tx = tx.WithContext(ctx)
			migrator := tx.Migrator()
			if !migrator.HasTable(&tables.TableCircuitBreakerConfig{}) {
				logger.Info("[configstore] %s: creating table TableCircuitBreakerConfig", migrationName)
				if err := migrator.CreateTable(&tables.TableCircuitBreakerConfig{}); err != nil {
					return err
				}
			}
			return nil
		...
		Rollback: func(tx *gorm.DB) error {
			tx = tx.WithContext(ctx)
			migrator := tx.Migrator()
			logger.Info("[configstore] %s: dropping table TableCircuitBreakerConfig", migrationName)
			if err := migrator.DropTable(&tables.TableCircuitBreakerConfig{}); err != nil {
				return err
			}
			return nil
		},
	...
	if err := m.Migrate(); err != nil {
		return fmt.Errorf("error while running add circuit breaker config table migration: %s", err.Error())
	}
	return nil
}

// migrationAddSCIMTables adds the scim_users and scim_groups tables backing the
// SSO/SCIM provisioning sync engine (Keycloak in this slice). Follows the
// add_guardrail_config_table migration pattern. These tables are inert until an
// SSO provider is configured and enabled — they hold no rows otherwise, so the
// migration is safe to apply to every deployment.
func migrationAddSCIMTables(ctx context.Context, db *gorm.DB, logger schemas.Logger) error {
	migrationName := "add_scim_tables"
	logger.Info("[configstore] starting migration %s", migrationName)
	defer logger.Info("[configstore] finished migration %s", migrationName)
	m := migrator.New(db, migrator.DefaultOptions, []*migrator.Migration{{
		ID: migrationName,
		Migrate: func(tx *gorm.DB) error {
			tx = tx.WithContext(ctx)
			migrator := tx.Migrator()
			if !migrator.HasTable(&tables.TableSCIMUser{}) {
				logger.Info("[configstore] %s: creating table TableSCIMUser", migrationName)
				if err := migrator.CreateTable(&tables.TableSCIMUser{}); err != nil {
					return err
				}
			}
			if !migrator.HasTable(&tables.TableSCIMGroup{}) {
				logger.Info("[configstore] %s: creating table TableSCIMGroup", migrationName)
				if err := migrator.CreateTable(&tables.TableSCIMGroup{}); err != nil {
					return err
				}
			}
			return nil
		...
		Rollback: func(tx *gorm.DB) error {
			tx = tx.WithContext(ctx)
			migrator := tx.Migrator()
			logger.Info("[configstore] %s: dropping tables TableSCIMUser, TableSCIMGroup", migrationName)
			if err := migrator.DropTable(&tables.TableSCIMGroup{}); err != nil {
				return err
			}
			if err := migrator.DropTable(&tables.TableSCIMUser{}); err != nil {
				return err
			}
			return nil
		},
	...
	if err := m.Migrate(); err != nil {
		return fmt.Errorf("error while running add scim tables migration: %s", err.Error())
	}
	return nil
}

// migrationAddAdminAPIKeyTables adds the governance_admin_api_keys and
// governance_admin_api_key_scopes tables backing the scope-based admin API keys
// feature. Keys are stored hashed (SHA-256 value_hash) with only a display
// prefix retained; scopes reuse the RBAC permission vocabulary. No rows are
// seeded — the feature is default-off by the absence of key rows. Follows the
// add_rbac_tables pattern.
func migrationAddAdminAPIKeyTables(ctx context.Context, db *gorm.DB, logger schemas.Logger) error {
	migrationName := "add_admin_api_key_tables"
	logger.Info("[configstore] starting migration %s", migrationName)
	defer logger.Info("[configstore] finished migration %s", migrationName)
	m := migrator.New(db, migrator.DefaultOptions, []*migrator.Migration{{
		ID: migrationName,
		Migrate: func(tx *gorm.DB) error {
			tx = tx.WithContext(ctx)
			migrator := tx.Migrator()
			// Keys before scopes because the children FK the key.
			if !migrator.HasTable(&tables.TableAdminAPIKey{}) {
				logger.Info("[configstore] %s: creating table TableAdminAPIKey", migrationName)
				if err := migrator.CreateTable(&tables.TableAdminAPIKey{}); err != nil {
					return err
				}
			}
			if !migrator.HasTable(&tables.TableAdminAPIKeyScope{}) {
				logger.Info("[configstore] %s: creating table TableAdminAPIKeyScope", migrationName)
				if err := migrator.CreateTable(&tables.TableAdminAPIKeyScope{}); err != nil {
					return err
				}
			}
			return nil
		...
		Rollback: func(tx *gorm.DB) error {
			tx = tx.WithContext(ctx)
			migrator := tx.Migrator()
			logger.Info("[configstore] %s: dropping admin API key tables", migrationName)
			if err := migrator.DropTable(&tables.TableAdminAPIKeyScope{}); err != nil {
				return err
			}
			if err := migrator.DropTable(&tables.TableAdminAPIKey{}); err != nil {
				return err
			}
			return nil
		},
	...
	if err := m.Migrate(); err != nil {
		return fmt.Errorf("error while running add admin api key tables migration: %s", err.Error())
	}
	return nil
}

// migrationAddAuditLogSettingsTable adds the governance_audit_log_settings
// singleton table backing audit-log retention and export. No row is seeded —
// the feature is default-off by the absence of the settings row (the retention
// worker no-ops and no export sink is installed). Follows the
// add_audit_log_table migration pattern.
func migrationAddAuditLogSettingsTable(ctx context.Context, db *gorm.DB, logger schemas.Logger) error {
	migrationName := "add_audit_log_settings_table"
	logger.Info("[configstore] starting migration %s", migrationName)
	defer logger.Info("[configstore] finished migration %s", migrationName)
	m := migrator.New(db, migrator.DefaultOptions, []*migrator.Migration{{
		ID: migrationName,
		Migrate: func(tx *gorm.DB) error {
			tx = tx.WithContext(ctx)
			migrator := tx.Migrator()
			if !migrator.HasTable(&tables.TableAuditLogSettings{}) {
				logger.Info("[configstore] %s: creating table TableAuditLogSettings", migrationName)
				if err := migrator.CreateTable(&tables.TableAuditLogSettings{}); err != nil {
					return err
				}
			}
			return nil
		...
		Rollback: func(tx *gorm.DB) error {
			tx = tx.WithContext(ctx)
			migrator := tx.Migrator()
			logger.Info("[configstore] %s: dropping table TableAuditLogSettings", migrationName)
			if err := migrator.DropTable(&tables.TableAuditLogSettings{}); err != nil {
				return err
			}
			return nil
		},
	...
	if err := m.Migrate(); err != nil {
		return fmt.Errorf("error while running add audit log settings table migration: %s", err.Error())
	}
	return nil
}

// migrationAddAlertChannelsTable adds the alert_channels table backing the
// alert-channels UI and the framework/alerting dispatcher. No row is seeded —
// the feature is default-off by the absence of channel rows (Publish is a
// no-op and no delivery goroutine touches the network). Follows the
// add_audit_log_settings_table migration pattern.
func migrationAddAlertChannelsTable(ctx context.Context, db *gorm.DB, logger schemas.Logger) error {
	migrationName := "add_alert_channels_table"
	logger.Info("[configstore] starting migration %s", migrationName)
	defer logger.Info("[configstore] finished migration %s", migrationName)
	m := migrator.New(db, migrator.DefaultOptions, []*migrator.Migration{{
		ID: migrationName,
		Migrate: func(tx *gorm.DB) error {
			tx = tx.WithContext(ctx)
			migrator := tx.Migrator()
			if !migrator.HasTable(&tables.TableAlertChannel{}) {
				logger.Info("[configstore] %s: creating table TableAlertChannel", migrationName)
				if err := migrator.CreateTable(&tables.TableAlertChannel{}); err != nil {
					return err
				}
			}
			return nil
		...
		Rollback: func(tx *gorm.DB) error {
			tx = tx.WithContext(ctx)
			migrator := tx.Migrator()
			logger.Info("[configstore] %s: dropping table TableAlertChannel", migrationName)
			if err := migrator.DropTable(&tables.TableAlertChannel{}); err != nil {
				return err
			}
			return nil
		},
	...
	if err := m.Migrate(); err != nil {
		return fmt.Errorf("error while running add alert channels table migration: %s", err.Error())
	}
	return nil
}

// migrationAddJWTAuthConfigTable adds the governance_jwt_auth_configs table
// backing data-plane JWT→virtual-key auth. No row is seeded — the feature is
// default-off by the absence of enabled rows (the transport middleware
// snapshot stays nil). Follows the add_alert_channels_table migration pattern.
func migrationAddJWTAuthConfigTable(ctx context.Context, db *gorm.DB, logger schemas.Logger) error {
	migrationName := "add_jwt_auth_config_table"
	logger.Info("[configstore] starting migration %s", migrationName)
	defer logger.Info("[configstore] finished migration %s", migrationName)
	m := migrator.New(db, migrator.DefaultOptions, []*migrator.Migration{{
		ID: migrationName,
		Migrate: func(tx *gorm.DB) error {
			tx = tx.WithContext(ctx)
			migrator := tx.Migrator()
			if !migrator.HasTable(&tables.TableJWTAuthConfig{}) {
				logger.Info("[configstore] %s: creating table TableJWTAuthConfig", migrationName)
				if err := migrator.CreateTable(&tables.TableJWTAuthConfig{}); err != nil {
					return err
				}
			}
			return nil
		...
		Rollback: func(tx *gorm.DB) error {
			tx = tx.WithContext(ctx)
			migrator := tx.Migrator()
			logger.Info("[configstore] %s: dropping table TableJWTAuthConfig", migrationName)
			if err := migrator.DropTable(&tables.TableJWTAuthConfig{}); err != nil {
				return err
			}
			return nil
		},
	...
	if err := m.Migrate(); err != nil {
		return fmt.Errorf("error while running add jwt auth config table migration: %s", err.Error())
	}
	return nil
}

// migrationAddSCIMUserMappingColumns adds the attribute-mapping outcome
// columns to scim_users: mapped_role_id,
// mapped_team_ids_json, mapped_business_unit_id. Purely additive (nullable /
// empty defaults); no rows are touched.
func migrationAddSCIMUserMappingColumns(ctx context.Context, db *gorm.DB, logger schemas.Logger) error {
	migrationName := "add_scim_user_mapping_columns"
	logger.Info("[configstore] starting migration %s", migrationName)
	defer logger.Info("[configstore] finished migration %s", migrationName)
	columns := []string{"MappedRoleID", "MappedTeamIDsJSON", "MappedBusinessUnitID"}
	m := migrator.New(db, migrator.DefaultOptions, []*migrator.Migration{{
		ID: migrationName,
		Migrate: func(tx *gorm.DB) error {
			tx = tx.WithContext(ctx)
			migrator := tx.Migrator()
			for _, column := range columns {
				if !migrator.HasColumn(&tables.TableSCIMUser{}, column) {
					logger.Info("[configstore] %s: adding column %s", migrationName, column)
					if err := migrator.AddColumn(&tables.TableSCIMUser{}, column); err != nil {
						return err
					}
				}
			}
		...
		Rollback: func(tx *gorm.DB) error {
			...
			for _, column := range columns {
				if migrator.HasColumn(&tables.TableSCIMUser{}, column) {
					if err := migrator.DropColumn(&tables.TableSCIMUser{}, column); err != nil {
						return err
					}
				}
			}
	...
	if err := m.Migrate(); err != nil {
		return fmt.Errorf("error while running add scim user mapping columns migration: %s", err.Error())
	}
	return nil
}

// migrationAddAuditLogTable adds the governance_audit_logs table backing the
// audit-logs UI. It is an append-only, tamper-evident trail of governance
// mutations. Follows the add_guardrail_config_table migration pattern.
func migrationAddAuditLogTable(ctx context.Context, db *gorm.DB, logger schemas.Logger) error {
	migrationName := "add_audit_log_table"
	logger.Info("[configstore] starting migration %s", migrationName)
	defer logger.Info("[configstore] finished migration %s", migrationName)
	m := migrator.New(db, migrator.DefaultOptions, []*migrator.Migration{{
		ID: migrationName,
		Migrate: func(tx *gorm.DB) error {
			tx = tx.WithContext(ctx)
			migrator := tx.Migrator()
			if !migrator.HasTable(&tables.TableAuditLog{}) {
				logger.Info("[configstore] %s: creating table TableAuditLog", migrationName)
				if err := migrator.CreateTable(&tables.TableAuditLog{}); err != nil {
					return err
				}
			}
			return nil
		...
		Rollback: func(tx *gorm.DB) error {
			tx = tx.WithContext(ctx)
			migrator := tx.Migrator()
			logger.Info("[configstore] %s: dropping table TableAuditLog", migrationName)
			if err := migrator.DropTable(&tables.TableAuditLog{}); err != nil {
				return err
			}
			return nil
		},
	...
	if err := m.Migrate(); err != nil {
		return fmt.Errorf("error while running add audit log table migration: %s", err.Error())
	}
	return nil
}

// migrationAddPIIRuleTable adds the governance_pii_rules table backing the
// loopback-guard PII redactor rules UI. Follows the add_guardrail_config_table
// migration pattern.
func migrationAddPIIRuleTable(ctx context.Context, db *gorm.DB, logger schemas.Logger) error {
	migrationName := "add_pii_rule_table"
	logger.Info("[configstore] starting migration %s", migrationName)
	defer logger.Info("[configstore] finished migration %s", migrationName)
	m := migrator.New(db, migrator.DefaultOptions, []*migrator.Migration{{
		ID: migrationName,
		Migrate: func(tx *gorm.DB) error {
			tx = tx.WithContext(ctx)
			migrator := tx.Migrator()
			if !migrator.HasTable(&tables.TablePIIRule{}) {
				logger.Info("[configstore] %s: creating table TablePIIRule", migrationName)
				if err := migrator.CreateTable(&tables.TablePIIRule{}); err != nil {
					return err
				}
			}
			return nil
		...
		Rollback: func(tx *gorm.DB) error {
			tx = tx.WithContext(ctx)
			migrator := tx.Migrator()
			logger.Info("[configstore] %s: dropping table TablePIIRule", migrationName)
			if err := migrator.DropTable(&tables.TablePIIRule{}); err != nil {
				return err
			}
			return nil
		},
	...
	if err := m.Migrate(); err != nil {
		return fmt.Errorf("error while running add pii rule table migration: %s", err.Error())
	}
	return nil
}

// migrationAddBusinessUnitTable adds the governance_business_units table backing
// the user & org management UI. Created before the users table because a user
// belongs to a business unit. Follows the add_guardrail_config_table pattern.
func migrationAddBusinessUnitTable(ctx context.Context, db *gorm.DB, logger schemas.Logger) error {
	migrationName := "add_business_unit_table"
	logger.Info("[configstore] starting migration %s", migrationName)
	defer logger.Info("[configstore] finished migration %s", migrationName)
	m := migrator.New(db, migrator.DefaultOptions, []*migrator.Migration{{
		ID: migrationName,
		Migrate: func(tx *gorm.DB) error {
			tx = tx.WithContext(ctx)
			migrator := tx.Migrator()
			if !migrator.HasTable(&tables.TableBusinessUnit{}) {
				logger.Info("[configstore] %s: creating table TableBusinessUnit", migrationName)
				if err := migrator.CreateTable(&tables.TableBusinessUnit{}); err != nil {
					return err
				}
			}
			return nil
		...
		Rollback: func(tx *gorm.DB) error {
			tx = tx.WithContext(ctx)
			migrator := tx.Migrator()
			logger.Info("[configstore] %s: dropping table TableBusinessUnit", migrationName)
			if err := migrator.DropTable(&tables.TableBusinessUnit{}); err != nil {
				return err
			}
			return nil
		},
	...
	if err := m.Migrate(); err != nil {
		return fmt.Errorf("error while running add business unit table migration: %s", err.Error())
	}
	return nil
}

// migrationAddUserTable adds the governance_users table backing the user & org
// management UI. Follows the add_guardrail_config_table pattern.
func migrationAddUserTable(ctx context.Context, db *gorm.DB, logger schemas.Logger) error {
	...
	m := migrator.New(db, migrator.DefaultOptions, []*migrator.Migration{{
		...
		Rollback: func(tx *gorm.DB) error {
			tx = tx.WithContext(ctx)
			migrator := tx.Migrator()
			logger.Info("[configstore] %s: dropping table TableUser", migrationName)
	...
	if err := m.Migrate(); err != nil {
		return fmt.Errorf("error while running add user table migration: %s", err.Error())
	}
	return nil
}

// migrationAddRBACTables adds the governance_roles, governance_permissions and
// governance_role_assignments tables backing the RBAC & access control UI, then
// seeds a built-in admin role carrying a single wildcard permission. The seed is
// idempotent (skipped when the admin role already exists) and creates the role
// only — never an assignment — because the local-admin (password) auth path
// always bypasses RBAC, so no assignment is needed to keep an operator from
// being locked out. Follows the add_user_table pattern.
func migrationAddRBACTables(ctx context.Context, db *gorm.DB, logger schemas.Logger) error {
	migrationName := "add_rbac_tables"
	logger.Info("[configstore] starting migration %s", migrationName)
	defer logger.Info("[configstore] finished migration %s", migrationName)
	m := migrator.New(db, migrator.DefaultOptions, []*migrator.Migration{{
		ID: migrationName,
		Migrate: func(tx *gorm.DB) error {
			tx = tx.WithContext(ctx)
			migrator := tx.Migrator()
			// Roles before permissions/assignments because the children FK the role.
			if !migrator.HasTable(&tables.TableRole{}) {
				logger.Info("[configstore] %s: creating table TableRole", migrationName)
				if err := migrator.CreateTable(&tables.TableRole{}); err != nil {
					return err
				}
			}
			if !migrator.HasTable(&tables.TablePermission{}) {
				logger.Info("[configstore] %s: creating table TablePermission", migrationName)
				if err := migrator.CreateTable(&tables.TablePermission{}); err != nil {
					return err
				}
			}
			if !migrator.HasTable(&tables.TableRoleAssignment{}) {
				logger.Info("[configstore] %s: creating table TableRoleAssignment", migrationName)
				if err := migrator.CreateTable(&tables.TableRoleAssignment{}); err != nil {
					return err
				}
			}
			// Seed the built-in admin role (idempotent).
			var existing int64
			if err := tx.Model(&tables.TableRole{}).
				Where("name = ?", tables.DefaultAdminRoleName).
				Count(&existing).Error; err != nil {
				return err
			}
			if existing == 0 {
				logger.Info("[configstore] %s: seeding built-in admin role", migrationName)
				now := time.Now()
				role := &tables.TableRole{
					ID:          uuid.NewString(),
					Name:        tables.DefaultAdminRoleName,
					Description: "Built-in full-access role for Loopback Gateway administrators.",
					IsSystem:    true,
					CreatedAt:   now,
					UpdatedAt:   now,
				}
				...
				perm := &tables.TablePermission{
					ID:        uuid.NewString(),
					RoleID:    role.ID,
					Resource:  tables.RbacWildcard,
					Operation: tables.RbacWildcard,
					CreatedAt: now,
				}
		...
		Rollback: func(tx *gorm.DB) error {
			tx = tx.WithContext(ctx)
			migrator := tx.Migrator()
			logger.Info("[configstore] %s: dropping RBAC tables", migrationName)
			if err := migrator.DropTable(&tables.TableRoleAssignment{}); err != nil {
				return err
			}
			if err := migrator.DropTable(&tables.TablePermission{}); err != nil {
				return err
			}
			if err := migrator.DropTable(&tables.TableRole{}); err != nil {
				return err
			}
			return nil
		},
	...
	if err := m.Migrate(); err != nil {
		return fmt.Errorf("error while running add rbac tables migration: %s", err.Error())
	}
	return nil
}
