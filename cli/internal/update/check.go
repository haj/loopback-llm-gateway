...
const (
	// No fork-owned download server exists yet, so update checks and
	// self-updates are disabled unless LOOPBACK_UPDATE_BASE_URL points at a
	// server exposing the /bifrost-cli/<version>/<os>/<arch>/<binary> layout.
	baseURLEnv     = "LOOPBACK_UPDATE_BASE_URL"
	checkInterval  = 24 * time.Hour
	requestTimeout = 3 * time.Second
)

// updateBaseURL returns the configured download server, or "" when update
// checks and self-updates are disabled.
...
func fetchLatestVersion() (string, error) {
	base := updateBaseURL()
	if base == "" {
		return "", fmt.Errorf("version check: no update server configured (set %s)", baseURLEnv)
	}
	client := &http.Client{Timeout: requestTimeout}
	resp, err := client.Get(base + "/bifrost-cli/latest/version.txt")
	if err != nil {
		return "", err
	}
