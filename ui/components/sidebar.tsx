...
const externalLinks = [
	...
	},
	{
		...
		url: "https://docs.loopback.gateway",
		icon: BooksIcon,
		strokeWidth: 1,
	},
];
...
// Production-setup upsell card removed for the open-source build.
...
export default function AppSidebar() {
	...
	// Upstream release-notes promo disabled for the open-source build (no external release check).
	...
	const hasJWTAuthAccess = useRbac(RbacResource.JWTAuth, RbacOperation.View);
		...
		() => [
			...
			},
			{
				...
				subItems: [
					...
					{
						title: "JWT Auth",
						url: "/workspace/jwt-auth",
						icon: KeyRound,
						description: "Map external IdP JWTs to virtual keys",
						hasAccess: hasJWTAuthAccess,
					},
				],
			},
			{
			...
			},
			{
				...
				subItems: [
					...
					},
					{
						...
						url: "/workspace/alert-channels",
						icon: Siren,
						description: "Slack, PagerDuty, and webhook alerts",
						hasAccess: hasAlertChannelsAccess,
					},
					{
						title: "Secure Setup",
						url: "/workspace/secure-setup",
						icon: ShieldCheck,
						description: "Lock down auth, CORS, and RBAC",
						hasAccess: hasRbacAccess,
					},
				],
			},
			...
			},
			{
				...
				icon: Network,
				description: "Manage Loopback Gateway cluster",
			...
			},
			{
				...
				icon: Settings2Icon,
				description: "Loopback Gateway settings",
	...
	const logoSrc = mounted && resolvedTheme === "dark" ? "/loopback-logo-dark.svg" : "/loopback-logo.svg";
	const iconSrc = "/loopback-icon.svg";
			...
			cards.push({
				...
				description: (
					<div className="flex h-full flex-col gap-2">
						<img src={newReleaseImage} alt="Loopback Gateway" className="h-[95px] rounded-md object-cover" />
						<a
							href={`https://docs.loopback.gateway/changelogs/${latestRelease.name}`}
							target="_blank"
							rel="noopener noreferrer"
			...
			<SidebarHeader className="mt-1 ml-2 flex justify-between px-0 group-data-[collapsible=icon]:ml-0 group-data-[collapsible=icon]:h-auto">
				...
				<div className="flex h-10 w-full items-center justify-between px-1.5 group-data-[collapsible=icon]:hidden">
					<Link to="/workspace/logs" className="group flex items-center gap-2 pl-2">
						<img className="h-[22px] w-auto" src={logoSrc} alt="Loopback Gateway" width={70} height={70} />
					</Link>
					<button
				...
				<div
					...
					onClick={toggleSidebar}
				>
					<img className="h-[22px] w-auto" src={iconSrc} alt="Loopback Gateway" width={22} height={22} style={{ width: 18 }} />
				</div>
			</SidebarHeader>
			{envLabel && (
