...
function authHelpText(authType?: MCPAuthType | string): string {
	switch (authType) {
		case "headers":
			return "Add the request headers Loopback Gateway should send with each tool call.";
		case "oauth":
		...
		<Sheet open={open} onOpenChange={(sheetOpen) => !sheetOpen && !oauthFlow && !headersFlow && onClose()}>
			<SheetContent className="flex w-full flex-col overflow-x-hidden p-0 pt-4 sm:max-w-2xl">
				<SheetHeader className="flex flex-col items-start px-0 py-4" headerClassName="mb-0 sticky px-8 -top-4 bg-card z-10">
					...
					<SheetDescription>Confirm the catalog configuration before adding this server to Loopback Gateway.</SheetDescription>
				</SheetHeader>
						...
						<div className="flex-1 space-y-6 px-8 pt-5 pb-6">
							...
							<section className="space-y-4">
								<div className="space-y-1">
									...
									<p className="text-muted-foreground text-sm">Loopback Gateway uses this name internally when routing MCP tool calls.</p>
								</div>
											...
											<TooltipProvider>
												<Tooltip>
													...
													<TooltipContent className="max-w-xs">
														<p>Leave a value blank to read it from the environment where Loopback Gateway runs.</p>
													</TooltipContent>
												</Tooltip>
											</TooltipProvider>
										</div>
										<p className="text-muted-foreground text-sm">Values used when Loopback Gateway starts this stdio MCP server.</p>
									</div>
									<HeadersTable
										value={envVars}
										onChange={setEnvVars}
