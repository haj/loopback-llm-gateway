...
type Bifrost struct {
	...
	// circuitBreakers holds the per-provider circuit breaker registry. nil until
	// a provider is explicitly configured (opt-in / default-off); the hot path
	// treats a nil registry as a complete no-op. See circuitbreaker.go.
	circuitBreakers atomic.Pointer[circuitBreakerRegistry]
	// circuitBreakerListener is the optional state-transition observer (alerting
	// bridge). nil default = hot path unchanged. See circuitbreaker.go.
	circuitBreakerListener atomic.Pointer[CircuitBreakerListener]
}
...
func (bifrost *Bifrost) handleRequest(ctx *schemas.BifrostContext, req *schemas.BifrostRequest) (*schemas.BifrostResponse, *schemas.BifrostError) {
	...
	// Circuit breaker (opt-in / default-off): if the primary provider's breaker
	// is OPEN, short-circuit to the fallback chain instead of calling the dead
	// provider. circuitBreakerAllow is a no-op (returns allowed=true) when the
	// breaker is unconfigured, so existing behavior is unchanged.
	var primaryResult *schemas.BifrostResponse
	var primaryErr *schemas.BifrostError
	if allowed, isProbe := bifrost.circuitBreakerAllow(provider); !allowed {
		primaryErr = bifrost.newCircuitBreakerOpenError(req, provider, model)
		ctx.AppendRoutingEngineLog(schemas.RoutingEngineCircuitBreaker, schemas.LogLevelWarn, fmt.Sprintf("Circuit breaker OPEN for %s; skipping primary and evaluating fallbacks", provider))
		bifrost.logger.Debug(fmt.Sprintf("circuit breaker open for provider %s; skipping primary attempt", provider))
	} else {
		primaryResult, primaryErr = bifrost.tryRequest(ctx, req)
		bifrost.circuitBreakerRecord(provider, primaryErr == nil, isProbe)
	}
	if primaryErr != nil {
...
func (bifrost *Bifrost) handleStreamRequest(ctx *schemas.BifrostContext, req *schemas.BifrostRequest) (chan *schemas.BifrostStreamChunk, *schemas.BifrostError) {
	...
	// Circuit breaker (opt-in / default-off): mirror handleRequest. A nil/unconfigured
	// breaker allows everything, so stream behavior is unchanged when off. Note: the
	// breaker observes the stream's initial connect outcome (primaryErr); mid-stream
	// failures after a successful open are not yet fed back (deferred refinement).
	var primaryResult chan *schemas.BifrostStreamChunk
	var primaryErr *schemas.BifrostError
	if allowed, isProbe := bifrost.circuitBreakerAllow(provider); !allowed {
		primaryErr = bifrost.newCircuitBreakerOpenError(req, provider, model)
		ctx.AppendRoutingEngineLog(schemas.RoutingEngineCircuitBreaker, schemas.LogLevelWarn, fmt.Sprintf("Circuit breaker OPEN for %s; skipping primary and evaluating fallbacks", provider))
		bifrost.logger.Debug(fmt.Sprintf("circuit breaker open for provider %s; skipping primary stream attempt", provider))
	} else {
