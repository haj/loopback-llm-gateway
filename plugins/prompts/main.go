...
func (p *Plugin) loadCache(ctx context.Context) error {
	...
	// Keep any deployment-aware resolver's routing table coherent with the same
	// reload. Non-fatal: a routing-table refresh failure must not stop prompt
	// injection from working off the (already updated) version cache.
	if dr, ok := p.resolver.(interface {
		reloadDeployments(context.Context) error
	}); ok {
		if err := dr.reloadDeployments(ctx); err != nil && p.logger != nil {
			p.logger.Warn("prompts plugin: failed to reload deployment routing table: %v", err)
		}
	}
	return nil
}
...
func (p *Plugin) HTTPTransportPreHook(ctx *schemas.BifrostContext, req *schemas.HTTPRequest) (*schemas.HTTPResponse, error) {
	...
	if d := strings.TrimSpace(req.CaseInsensitiveHeaderLookup(PromptDeploymentHeader)); d != "" {
		ctx.SetValue(PromptDeploymentKey, d)
	}
	return nil, nil
}
