...
type GovernancePlugin struct {
	...
	// violationListener is the optional threshold-violation observer (alerting
	// bridge). nil default = deny paths unchanged. See SetViolationListener.
	violationListener atomic.Pointer[ViolationListener]
}

// ViolationInfo describes one governance threshold violation (budget, rate,
// token, or request limit) for the optional violation listener.
...
// ViolationListener observes governance threshold violations (the alerting
// bridge in the HTTP transport). Invoked on its own goroutine — never on the
// request path — so implementations may do I/O.
type ViolationListener func(info ViolationInfo)

// SetViolationListener installs (or, with nil, removes) the violation
// listener. ADDITIVE and DEFAULT-OFF: with no listener the deny paths are
// byte-for-byte unchanged.
func (p *GovernancePlugin) SetViolationListener(listener ViolationListener) {
	if p == nil {
		return
	}
	if listener == nil {
		p.violationListener.Store(nil)
		return
	}
	p.violationListener.Store(&listener)
}

// notifyViolation offers a threshold violation to the listener, nil-safely and
// on a fresh goroutine so the deny path never blocks on alerting.
func (p *GovernancePlugin) notifyViolation(result *EvaluationResult, evaluationRequest *EvaluationRequest) {
	if p == nil || result == nil {
		return
	}
	lp := p.violationListener.Load()
	if lp == nil {
		return
	}
	info := ViolationInfo{
	...
	if evaluationRequest != nil {
		info.Provider = evaluationRequest.Provider
		info.Model = evaluationRequest.Model
	}
	if result.VirtualKey != nil {
		info.VirtualKeyID = result.VirtualKey.ID
		info.VirtualKeyName = result.VirtualKey.Name
	}
	listener := *lp
	go listener(info)
}
...
func (p *GovernancePlugin) EvaluateGovernanceRequest(ctx *schemas.BifrostContext, evaluationRequest *EvaluationRequest, requestType schemas.RequestType) (*EvaluationResult, *schemas.BifrostError) {
	...
	case DecisionRateLimited, DecisionTokenLimited, DecisionRequestLimited:
		p.notifyViolation(result, evaluationRequest)
		return result, &schemas.BifrostError{
			Type:       new(string(result.Decision)),
			StatusCode: new(429),
			Error: &schemas.ErrorField{
