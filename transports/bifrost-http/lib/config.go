...
import (
	...
	"github.com/maximhq/bifrost/plugins/loopbackkafka"
...
var builtinPluginNames = []string{
	...
	compat.PluginName,
	maxim.PluginName,
	loopbackguard.PluginName,
	loopbackkafka.PluginName,
}
...
type ConfigData struct {
	...
	// SCIMConfig holds the optional SSO/SCIM section (scim_config). It is
	// default-OFF: nil or {enabled:false} leaves all SSO/JWT machinery inert so
	// existing password/session auth is unaffected. Keycloak is the only
	// implemented provider in this slice.
	SCIMConfig *sso.SCIMConfig `json:"scim_config,omitempty"`
}
...
type Config struct {
	...
	// SCIMConfig is the effective SSO/SCIM configuration (Keycloak in this
	// slice). Nil or disabled means SSO is off — the default.
	SCIMConfig *sso.SCIMConfig
	// SSOValidator validates IdP-issued JWTs against the provider JWKS. Nil when
	// SSO is disabled or misconfigured; the auth middleware skips the JWT branch
	// entirely when nil, so password/session auth is never affected.
	SSOValidator *sso.Validator

	// Vault bundles the OSS vault/secret-manager registry (framework/vault),
	// its effective config, and hot-refresh state. Nil when
	// config_store.vault_store is absent or disabled — the default — in which
	// case the schemas.Vault*Hook seam stays nil and secret handling is
...
func LoadConfig(ctx context.Context, configDirPath string) (*Config, error) {
	...
	// 1a. Vault / secret-manager sync (config_store.vault_store). Must run
	// before initStores so configstore AfterFind hooks can resolve vault refs.
	// Default-OFF: without config all schemas.Vault*Hooks stay nil.
	if err := initVault(config, &configData); err != nil {
		return nil, err
	}
	...
	// 10b. SSO/SCIM (scim_config). Default-OFF, fail-safe: never blocks startup.
	loadSSOConfig(ctx, config, &configData)
...
// initVault wires the OSS vault/secret-manager registry (framework/vault) into
// the core schemas hook seam from config_store.vault_store. Vault configuration
// is file/env-only (like encryption_key) because config-store rows may
// themselves hold vault references.
//
// Default-OFF invariant: when the section is absent or disabled, this returns
// without touching the four schemas.Vault*Hook globals (all nil), so provider
// key handling is byte-for-byte unchanged. This is the ONLY place the hooks
// are wired. The store/remove hooks are wired only in read_and_write mode;
// read_only deployments resolve existing refs but never rewrite stored rows.
func initVault(config *Config, configData *ConfigData) error {
	var vaultCfg *vault.Config
	if configData.ConfigStoreConfig != nil {
		vaultCfg = configData.ConfigStoreConfig.VaultStore
	}
	if vaultCfg == nil || !vaultCfg.Enabled {
		return nil
	}
	registry, err := vault.NewRegistry(vaultCfg, logger)
	if err != nil {
		return fmt.Errorf("failed to initialize vault store: %w", err)
	}
	schemas.VaultResolveHook = registry.ResolveString
	schemas.VaultPrefixHook = registry.Prefix
	if vaultCfg.WriteEnabled() {
		schemas.VaultStoreHook = registry.StoreString
		schemas.VaultRemoveHook = registry.Remove
	} else {
		// Idempotency: a previous (test) wiring must not leak write hooks into
		// a read_only re-init.
		schemas.VaultStoreHook = nil
		schemas.VaultRemoveHook = nil
	}
	config.Vault = &VaultRuntime{Registry: registry, Config: vaultCfg}
	if logger != nil {
		logger.Info("vault store initialized (backend %s, prefix %s, access_mode %s)",
			vaultCfg.Type, vaultCfg.EffectivePrefix(), vaultCfg.EffectiveAccessMode())
	}
	return nil
}
