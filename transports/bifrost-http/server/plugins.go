...
func loadBuiltinPlugin(ctx context.Context, name string, pluginConfig any, bifrostConfig *lib.Config) (schemas.BasePlugin, error) {
	...
	case prompts.PluginName:
		// Use a deployment-aware resolver so prompt deployments can split live
		// traffic across versions (weighted, with a latest-version fallback when a
		// pinned version is deleted). It wraps the default header resolver, so an
		// explicit x-bf-prompt-version still bypasses deployment routing.
		deploymentResolver := prompts.NewDeploymentResolver(nil, bifrostConfig.ConfigStore, logger)
		return prompts.InitWithResolver(ctx, bifrostConfig.ConfigStore, deploymentResolver, logger)
	...
	case loopbackguard.PluginName:
		guardConfig, err := MarshalPluginConfig[loopbackguard.Config](pluginConfig)
		if err != nil {
			return nil, fmt.Errorf("failed to marshal loopback-guard plugin config: %w", err)
		}
		return loopbackguard.Init(guardConfig, logger)

	case loopbackkafka.PluginName:
		kafkaConfig, err := MarshalPluginConfig[loopbackkafka.Config](pluginConfig)
		if err != nil {
			return nil, fmt.Errorf("failed to marshal loopback-kafka plugin config: %w", err)
		}
		return loopbackkafka.Init(ctx, kafkaConfig, logger)
...
func (s *BifrostHTTPServer) loadBuiltinPlugins(ctx context.Context) error {
	...
	// 9. Loopback Guard (if configured in PluginConfigs). Guardrail/redaction
	// engine: registered as a built-in so it can be enabled and configured from
	// the UI. Default-off — absent or disabled config means it is not loaded, and
	// an empty config makes it a no-op that blocks nothing.
	loopbackGuardConfig := s.getPluginConfig(loopbackguard.PluginName)
	if loopbackGuardConfig != nil && loopbackGuardConfig.Enabled {
		s.registerPluginWithStatus(ctx, loopbackguard.PluginName, nil, loopbackGuardConfig.Config, false)
	} else {
		s.markPluginDisabled(loopbackguard.PluginName)
	}
	s.Config.SetPluginOrderInfo(loopbackguard.PluginName, builtinPlacement, schemas.Ptr(9))

	// 10. Loopback Kafka data connector (if configured in PluginConfigs). Observability
	// connector that publishes completed request/response telemetry to a Kafka topic.
	// Default-off — absent or disabled config means it is not loaded. Receives traces
	// via the ObservabilityPlugin.Inject path, independent of its placement order.
	loopbackKafkaConfig := s.getPluginConfig(loopbackkafka.PluginName)
	if loopbackKafkaConfig != nil && loopbackKafkaConfig.Enabled {
		s.registerPluginWithStatus(ctx, loopbackkafka.PluginName, nil, loopbackKafkaConfig.Config, false)
	} else {
		s.markPluginDisabled(loopbackkafka.PluginName)
	}
	s.Config.SetPluginOrderInfo(loopbackkafka.PluginName, builtinPlacement, schemas.Ptr(10))
