...
type BifrostHTTPServer struct {
	...
	// AlertDispatcher is the framework/alerting delivery pipeline (nil without
	// a config store). AuditRetentionWorker prunes the audit trail per the
	// persisted settings row. Both are default-off by data absence.
	AlertDispatcher      *alerting.Dispatcher
	AuditRetentionWorker *handlers.AuditRetentionWorker

	// JWTVKAuthMiddleware maps external-IdP JWTs to virtual keys on inference
	// routes. Constructed in RegisterAPIRoutes, appended to the inference
	// middleware chain in Bootstrap. Default-off by data absence (nil config
	// snapshot).
	JWTVKAuthMiddleware *handlers.JWTVKAuthMiddleware
...
func (s *BifrostHTTPServer) RegisterAPIRoutes(ctx context.Context, callbacks ServerCallbacks, middlewares ...schemas.BifrostHTTPMiddleware) error {
	...
	// Guardrails handler: CRUD over guardrail configs that pushes the rebuilt
	// guardrail list into the live loopback-guard plugin. The plugin is resolved
	// per call so /api/plugins reloads are honored.
	var guardrailsHandler *handlers.GuardrailsHandler
	if s.Config.ConfigStore != nil {
		guardrailsHandler, err = handlers.NewGuardrailsHandler(s.Config.ConfigStore, func() *loopbackguard.Plugin {
			p, ferr := lib.FindPluginAs[*loopbackguard.Plugin](s.Config, loopbackguard.PluginName)
			if ferr != nil {
				return nil
			}
			return p
	...
	// PII rules handler: CRUD over PII redaction rules that pushes the rebuilt
	// Redactor (regex rules) and Presidio transformer set (presidio rules) into
	// the live loopback-guard plugin. The plugin is resolved per call so
	// /api/plugins reloads are honored.
	var piiRulesHandler *handlers.PIIRulesHandler
	if s.Config.ConfigStore != nil {
		piiRulesHandler, err = handlers.NewPIIRulesHandler(s.Config.ConfigStore, func() *loopbackguard.Plugin {
			p, ferr := lib.FindPluginAs[*loopbackguard.Plugin](s.Config, loopbackguard.PluginName)
			if ferr != nil {
				return nil
			}
		...
		if err != nil {
			return fmt.Errorf("failed to initialize pii rules handler: %v", err)
		}
	}
	// User & org management handler: CRUD over managed users and flat business
	// units, reusing the shared governance budget / rate-limit tables.
	var userManagementHandler *handlers.UserManagementHandler
	if s.Config.ConfigStore != nil {
		userManagementHandler, err = handlers.NewUserManagementHandler(s.Config.ConfigStore)
		if err != nil {
			return fmt.Errorf("failed to initialize user management handler: %v", err)
		}
	}
	// MCP tool groups handler: CRUD over named, scoped groups of MCP tool
	// references. The persisted rows are consumed by MCP visibility filtering
	// (EntityMCPToolGroup) at request time, so mutations need no live reload.
	var mcpToolGroupsHandler *handlers.MCPToolGroupsHandler
	if s.Config.ConfigStore != nil {
		mcpToolGroupsHandler, err = handlers.NewMCPToolGroupsHandler(s.Config.ConfigStore)
		if err != nil {
			return fmt.Errorf("failed to initialize mcp tool groups handler: %v", err)
		}
	}
	// Prompt deployments handler: CRUD over named, weighted version-routing
	// strategies for a prompt. Mutations refresh the prompts plugin cache (which
	// rebuilds the deployment routing table) via the same reloader the prompts
	// handler uses, so live traffic routing reflects changes without a restart.
	var promptDeploymentsHandler *handlers.PromptDeploymentsHandler
	if s.Config.ConfigStore != nil {
		promptDeploymentsHandler, err = handlers.NewPromptDeploymentsHandler(s.Config.ConfigStore, promptsReloader)
		if err != nil {
			return fmt.Errorf("failed to initialize prompt deployments handler: %v", err)
		}
	}
	// Circuit breaker handler: CRUD over per-provider circuit-breaker policies.
	// On mutation it pushes the policy into the live core engine via
	// Bifrost.ConfigureCircuitBreaker so the breaker enforces it without a
	// restart. Opt-in / default-off: with no policy rows the engine hot path is a
	// no-op. The client is resolved per call to avoid a stale reference.
	var circuitBreakerHandler *handlers.CircuitBreakerHandler
	if s.Config.ConfigStore != nil {
		circuitBreakerHandler, err = handlers.NewCircuitBreakerHandler(s.Config.ConfigStore, func() *bifrost.Bifrost {
			return s.Client
		})
		if err != nil {
			return fmt.Errorf("failed to initialize circuit breaker handler: %v", err)
		}
		// Re-apply persisted policies to the freshly-built engine on startup.
		circuitBreakerHandler.ApplyAllToEngine(context.Background())
	}
	// JWT auth: external-IdP JWTs on the data plane, mapped to virtual keys.
	// The middleware snapshot is nil until an enabled issuer row exists
	// (default-off); the handler rebuilds it on every mutation.
	var jwtAuthHandler *handlers.JWTAuthHandler
	if s.Config.ConfigStore != nil {
		s.JWTVKAuthMiddleware = handlers.NewJWTVKAuthMiddleware(s.Config.ConfigStore)
		jwtAuthHandler, err = handlers.NewJWTAuthHandler(s.Config.ConfigStore, s.JWTVKAuthMiddleware)
		if err != nil {
			return fmt.Errorf("failed to initialize jwt auth handler: %v", err)
		}
		// Push persisted issuer configs into the live snapshot on startup.
		jwtAuthHandler.ApplyAll(context.Background())
	}
	// RBAC middleware: constructed BEFORE the handlers that mirror or mutate
	// its runtime state (RBACHandler.getMyPermissions, SecureSetupHandler
	// enforce). Reads the persisted enforcement flag at construction; fail-open
	// on a missing key or store error.
	var rbacMiddleware *handlers.RBACMiddleware
	if s.Config.ConfigStore != nil {
		rbacMiddleware = handlers.NewRBACMiddleware(s.Config.ConfigStore)
	}
	// RBAC handler: CRUD over roles (with inline permissions) and role
	// assignments that bind a role to a managed user.
	var rbacHandler *handlers.RBACHandler
	if s.Config.ConfigStore != nil {
		rbacHandler, err = handlers.NewRBACHandler(s.Config.ConfigStore, rbacMiddleware)
		if err != nil {
			return fmt.Errorf("failed to initialize rbac handler: %v", err)
		}
	}
	// Secure-setup handler: insecure-defaults detection + one-click RBAC
	// enforcement. Additive and opt-in; the local-admin bypass in the RBAC
	// middleware guarantees enforce can never lock the operator out.
	var secureSetupHandler *handlers.SecureSetupHandler
	if s.Config.ConfigStore != nil {
		secureSetupHandler, err = handlers.NewSecureSetupHandler(s.Config.ConfigStore, rbacMiddleware, func() *configstore.ClientConfig {
			if s.Config == nil {
				return nil
			}
			return s.Config.ClientConfig
		})
		if err != nil {
			return fmt.Errorf("failed to initialize secure setup handler: %v", err)
		}
	}
	// Admin API keys handler: CRUD + rotate/revoke over scope-based admin-plane
	// API keys ("lbk_" bearer tokens for the management API — distinct from
	// governance virtual keys). Keys are hashed at rest; the plaintext is
	// returned exactly once on create/rotate. Default-off: with no key rows the
	// request path is unchanged (see APIKeyMiddleware wiring below).
	var apiKeysHandler *handlers.APIKeysHandler
	if s.Config.ConfigStore != nil {
		apiKeysHandler, err = handlers.NewAPIKeysHandler(s.Config.ConfigStore)
		if err != nil {
			return fmt.Errorf("failed to initialize api keys handler: %v", err)
		}
	}
	// SCIM/SSO handler: public login-page SSO config + admin provisioning
	// inspection and manual sync (Keycloak in this slice). Default-OFF: reports
	// disabled until scim_config is configured & enabled. scimResolver reads the
	// live config so a reload is honored.
	var scimHandler *handlers.SCIMHandler
	if s.Config != nil {
		scimHandler, err = handlers.NewSCIMHandler(s.Config.ConfigStore, func() *sso.SCIMConfig {
			if s.Config == nil {
				return nil
			}
			return s.Config.SCIMConfig
		})
		if err != nil {
			return fmt.Errorf("failed to initialize scim handler: %v", err)
		}
	}
	// Inbound SCIM 2.0 provisioning endpoint (/scim/v2). Default-off: 404s
	// until scim_config.provisioning is enabled with a bearer token; enforces
	// its own constant-time bearer auth (the prefix is whitelisted past
	// session auth in APIMiddleware).
	var scimV2Handler *handlers.SCIMV2Handler
	if s.Config != nil && s.Config.ConfigStore != nil {
		scimV2Handler, err = handlers.NewSCIMV2Handler(s.Config.ConfigStore, func() *sso.SCIMConfig {
			if s.Config == nil {
				return nil
			}
			return s.Config.SCIMConfig
		...
		if err != nil {
			return fmt.Errorf("failed to initialize scim v2 handler: %v", err)
		}
	}
	// Audit logs handler: read-only access to the append-only governance audit
	// trail. Mutation points across the governance handlers append signed events
	// via the shared recordAudit helper; this handler exposes querying them.
	var auditLogsHandler *handlers.AuditLogsHandler
	if s.Config.ConfigStore != nil {
		auditLogsHandler, err = handlers.NewAuditLogsHandler(s.Config.ConfigStore)
		if err != nil {
			return fmt.Errorf("failed to initialize audit logs handler: %v", err)
		}
		// Apply the persisted export settings (default-off: a missing settings
		// row installs no sink) and start the scheduled retention worker (no-op
		// per run while unconfigured). Both run entirely off the request path.
		handlers.LoadAuditExportSettings(ctx, s.Config.ConfigStore)
		s.AuditRetentionWorker = auditLogsHandler.RetentionWorker()
		s.AuditRetentionWorker.Start()
	}
	// Alerting: dispatcher + alert-channels handler + event-source bridges.
	// Default-off end to end: with zero alert_channels rows, Publish is a
	// single atomic load and every bridge below is a no-op.
	var alertChannelsHandler *handlers.AlertChannelsHandler
	if s.Config.ConfigStore != nil {
		s.AlertDispatcher = alerting.NewDispatcher(s.Config.ConfigStore, logger)
		s.AlertDispatcher.Start(ctx)
		// recordAudit publishes audit.mutation events through this seam.
		handlers.SetAlertPublisher(s.AlertDispatcher)
		alertChannelsHandler, err = handlers.NewAlertChannelsHandler(s.Config.ConfigStore, s.AlertDispatcher)
		if err != nil {
			return fmt.Errorf("failed to initialize alert channels handler: %v", err)
		}
		// Circuit-breaker bridge: state transitions become alert events. The
		// core invokes the listener on its own goroutine after releasing the
		// breaker mutex, and Publish is non-blocking, so the hot path never
		// waits on alerting. half_open is a probe detail, not alert-worthy.
		if s.Client != nil {
			dispatcher := s.AlertDispatcher
			s.Client.SetCircuitBreakerListener(func(provider schemas.ModelProvider, from, to bifrost.CircuitState) {
				var event alerting.Event
				switch to {
				case bifrost.CircuitOpen:
					event = alerting.Event{
						Type:     alerting.EventTypeCircuitBreakerOpen,
						Severity: alerting.SeverityCritical,
						Title:    fmt.Sprintf("Circuit breaker OPEN for %s", provider),
						Message:  fmt.Sprintf("Provider %s tripped its circuit breaker (%s -> %s); requests now fail over to fallbacks.", provider, from, to),
					}
				case bifrost.CircuitClosed:
					event = alerting.Event{
						Type:     alerting.EventTypeCircuitBreakerClosed,
						Severity: alerting.SeverityInfo,
						Title:    fmt.Sprintf("Circuit breaker recovered for %s", provider),
						Message:  fmt.Sprintf("Provider %s circuit breaker closed (%s -> %s); traffic restored.", provider, from, to),
					}
				default:
					return
				}
				event.Provider = string(provider)
				event.DedupKey = "circuit_breaker|" + string(provider) + "|" + string(to)
				event.Fields = map[string]string{"provider": string(provider), "from": string(from), "to": string(to)}
				dispatcher.Publish(event)
			})
		}
		// Governance bridge: budget / rate-limit violations become alert
		// events. Optional-interface assertion so enterprise governance
		// wrappers without the setter degrade silently.
		if governancePlugin, govErr := s.getGovernancePlugin(); govErr == nil {
			if setter, ok := governancePlugin.(interface {
				SetViolationListener(governance.ViolationListener)
			}); ok {
				dispatcher := s.AlertDispatcher
				setter.SetViolationListener(func(info governance.ViolationInfo) {
					eventType := alerting.EventTypeRateLimitExceeded
					severity := alerting.SeverityWarning
					if info.Decision == governance.DecisionBudgetExceeded {
						eventType = alerting.EventTypeBudgetExceeded
						severity = alerting.SeverityCritical
					}
					vkLabel := info.VirtualKeyName
					if vkLabel == "" {
						vkLabel = info.VirtualKeyID
					}
					dispatcher.Publish(alerting.Event{
						Type:     eventType,
						Severity: severity,
						Title:    fmt.Sprintf("Governance violation: %s", info.Decision),
						Message:  info.Reason,
						Provider: string(info.Provider),
						// Deduped per (decision, virtual key): violations repeat on
						// every rejected request, so one alert per window.
						DedupKey: string(info.Decision) + "|vk:" + info.VirtualKeyID,
						Fields: map[string]string{
							"decision":         string(info.Decision),
							"virtual_key_id":   info.VirtualKeyID,
							"virtual_key_name": vkLabel,
							"provider":         string(info.Provider),
							"model":            info.Model,
						},
					})
	...
	// RBAC authorization middleware: gates mutating governance/config/provider
	// routes. Fail-open by default (see handlers.NewRBACMiddleware) so this never
	// bricks an existing deployment. mutatingMiddlewares appends it to the base
	// chain only for the management handlers below. This MUST be the same
	// instance handed to RBACHandler/SecureSetupHandler above, or the one-click
	// enforce toggle would flip an instance that is not in the chain.
	mutatingMiddlewares := middlewares
	if rbacMiddleware != nil {
		mutatingMiddlewares = append(append([]schemas.BifrostHTTPMiddleware{}, middlewares...), rbacMiddleware.Middleware())
	}
	// Going ahead with API handlers
	healthHandler.RegisterRoutes(s.Router, middlewares...)
	// Provider and config management are gated by RBAC on mutating requests.
	providerHandler.RegisterRoutes(s.Router, mutatingMiddlewares...)
	...
	configHandler.RegisterRoutes(s.Router, mutatingMiddlewares...)
	...
	if governanceHandler != nil {
		governanceHandler.RegisterRoutes(s.Router, mutatingMiddlewares...)
	}
	if guardrailsHandler != nil {
		guardrailsHandler.RegisterRoutes(s.Router, mutatingMiddlewares...)
	}
	if piiRulesHandler != nil {
		piiRulesHandler.RegisterRoutes(s.Router, mutatingMiddlewares...)
	}
	if userManagementHandler != nil {
		userManagementHandler.RegisterRoutes(s.Router, mutatingMiddlewares...)
	}
	if mcpToolGroupsHandler != nil {
		mcpToolGroupsHandler.RegisterRoutes(s.Router, mutatingMiddlewares...)
	}
	if promptDeploymentsHandler != nil {
		promptDeploymentsHandler.RegisterRoutes(s.Router, mutatingMiddlewares...)
	}
	if circuitBreakerHandler != nil {
		circuitBreakerHandler.RegisterRoutes(s.Router, mutatingMiddlewares...)
	}
	if jwtAuthHandler != nil {
		jwtAuthHandler.RegisterRoutes(s.Router, mutatingMiddlewares...)
	}
	if rbacHandler != nil {
		rbacHandler.RegisterRoutes(s.Router, mutatingMiddlewares...)
	}
	if secureSetupHandler != nil {
		secureSetupHandler.RegisterRoutes(s.Router, mutatingMiddlewares...)
	}
	if apiKeysHandler != nil {
		apiKeysHandler.RegisterRoutes(s.Router, mutatingMiddlewares...)
	}
	if scimHandler != nil {
		scimHandler.RegisterRoutes(s.Router, mutatingMiddlewares...)
	}
	if scimV2Handler != nil {
		scimV2Handler.RegisterRoutes(s.Router, middlewares...)
	}
	if auditLogsHandler != nil {
		auditLogsHandler.RegisterRoutes(s.Router, middlewares...)
		// Settings update + manual prune change state: RBAC-gated.
		auditLogsHandler.RegisterMutatingRoutes(s.Router, mutatingMiddlewares...)
	}
	if alertChannelsHandler != nil {
		alertChannelsHandler.RegisterRoutes(s.Router, middlewares...)
		alertChannelsHandler.RegisterMutatingRoutes(s.Router, mutatingMiddlewares...)
	}
...
func (s *BifrostHTTPServer) Bootstrap(ctx context.Context) error {
	...
	if s.Config.ConfigStore == nil {
		...
		// Install the optional SSO/JWT validator (Keycloak in this slice). Nil
		// when SSO is disabled/misconfigured, which hard-disables the JWT path so
		// password/session auth is never affected.
		if s.Config != nil && s.Config.SSOValidator != nil {
			s.AuthMiddleware.SetSSOValidator(s.Config.SSOValidator)
		}
		if ctx.Value(schemas.BifrostContextKeyIsEnterprise) == nil {
			// Admin API key middleware runs immediately BEFORE the auth
			// middleware: it engages only when a "lbk_" bearer token is
			// presented (valid + in-scope → marks the request so APIMiddleware
			// skips; anything else falls through so APIMiddleware behaves
			// exactly as today). It is deliberately NOT added to
			// inferenceMiddlewares — inference auth stays owned by the
			// governance plugin's virtual keys.
			apiMiddlewares = append(apiMiddlewares, handlers.NewAPIKeyMiddleware(s.Config.ConfigStore).Middleware())
	...
	// JWT→virtual-key auth runs right after auth: with no enabled issuer rows
	// its snapshot is nil and the chain is one atomic load longer, nothing
	// more. It must precede the inference handlers so lib.ConvertToBifrostContext
	// sees the injected x-bf-vk header.
	if s.JWTVKAuthMiddleware != nil {
		inferenceMiddlewares = append(inferenceMiddlewares, s.JWTVKAuthMiddleware.Middleware())
	}
...
func (s *BifrostHTTPServer) Start() error {
	...
	case sig := <-sigChan:
		...
		go func() {
			...
			if s.AuditRetentionWorker != nil {
				logger.Info("stopping audit retention worker...")
				s.AuditRetentionWorker.Stop()
			}
			// Flush and close the audit export sink, then the alert dispatcher.
			handlers.ShutdownAuditExport()
			if s.AlertDispatcher != nil {
				logger.Info("stopping alert dispatcher...")
				s.AlertDispatcher.Stop()
			}
			if s.WSTicketStore != nil {
