...
type AuthMiddleware struct {
	...
	// ssoValidator validates IdP-issued JWTs (Keycloak in this slice). It is an
	// ADDITIONAL, default-OFF auth path: nil means SSO is disabled and the JWT
	// branch is skipped entirely, so password/session auth is unaffected. Held
	// behind an atomic.Pointer so config reloads can swap it without a restart.
	ssoValidator atomic.Pointer[sso.Validator]
}
...
// SetSSOValidator installs (or clears, with nil) the IdP JWT validator. Calling
// it with nil hard-disables the SSO/JWT auth path, guaranteeing operators can
// always fall back to password auth.
func (m *AuthMiddleware) SetSSOValidator(v *sso.Validator) {
	m.ssoValidator.Store(v)
}

// trySSOAuth attempts to authenticate a Bearer token as an IdP-issued JWT. It
// returns true only when SSO is configured AND the token is a valid IdP JWT that
// maps to a managed user; in that case it has already set the request context
// and the caller must stop. It returns false (without writing a response) for
// every other case — no validator, not a JWT, invalid signature/issuer/expiry —
// so the caller falls through to the existing password/session auth paths. This
// ordering makes SSO strictly additive and never able to reject a request that
// password auth would have accepted.
func (m *AuthMiddleware) trySSOAuth(ctx *fasthttp.RequestCtx, token string) bool {
	validator := m.ssoValidator.Load()
	if validator == nil {
		return false
	}
	// A JWT has three dot-separated segments; cheaply skip non-JWT bearer tokens
	// (session tokens, base64 admin creds) without invoking the validator.
	...
	// Map the verified identity to a managed Wave-2 user (just-in-time
	// provisioning), reusing the same logic the SCIM sync engine uses. The
	// provider's attribute mappings apply here too, so a JWT login picks up
	// role/team/BU placement without waiting for a directory sync.
	managed, err := sso.ProvisionFromIdentity(context.Background(), m.store, identity, true, validator.Mappings())
	if err != nil {
		logger.Error("[sso] valid JWT but failed to provision managed user: %v", err)
		return false
	}
	// Attach the resolved identity to the request. SSO users are NOT marked local
	// admin (that flag is reserved for password admin auth); RBAC runs normally.
	ctx.SetUserValue(schemas.BifrostContextKeyUserID, managed.ID)
	ctx.SetUserValue(schemas.BifrostContextKeyUserName, managed.Name)
	ctx.SetUserValue(schemas.BifrostContextKeySCIMClaims, identity.Raw)
...
func (m *AuthMiddleware) APIMiddleware() schemas.BifrostHTTPMiddleware {
	...
	whitelistedPrefixes := []string{
		...
		"/api/skills/serve/",
		// Inbound SCIM provisioning: IdPs (Okta/Entra/Keycloak) cannot hold
		// dashboard sessions, so this prefix skips session auth. SCIMV2Handler
		// enforces its OWN bearer auth (constant-time compare) on EVERY request
		// and 404s while provisioning is unconfigured — never add routes under
		// this prefix that skip requireProvisioningAuth.
		"/scim/v2/",
	}
	return m.middleware(func(authConfig *configstore.AuthConfig, url string) bool {
...
func (m *AuthMiddleware) middleware(shouldSkip func(*configstore.AuthConfig, string) bool) schemas.BifrostHTTPMiddleware {
	return func(next fasthttp.RequestHandler) fasthttp.RequestHandler {
		return func(ctx *fasthttp.RequestCtx) {
			...
			if scheme == "Bearer" {
				// SSO/JWT path (ADDITIONAL, default-OFF): if an IdP JWT validator
				// is configured and this Bearer token is a valid IdP JWT, accept it
				// and map it to a managed user. trySSOAuth never rejects — it
				// returns false for non-JWT/invalid tokens so password & session
				// auth below remain fully intact.
				if m.trySSOAuth(ctx, token) {
					next(ctx)
					return
				}
