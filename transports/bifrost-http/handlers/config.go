...
func (h *ConfigHandler) RegisterRoutes(r *router.Router, middlewares ...schemas.BifrostHTTPMiddleware) {
	...
	r.GET("/api/large-payload-config", lib.ChainMiddlewares(h.getLargePayloadConfig, middlewares...))
	r.PUT("/api/large-payload-config", lib.ChainMiddlewares(h.updateLargePayloadConfig, middlewares...))
...
// minLargePayloadBytes is the smallest accepted value (1 KB) for any
// large-payload byte threshold. Mirrors the UI-side validation.
const minLargePayloadBytes int64 = 1024

// getLargePayloadConfig handles GET /api/large-payload-config - Get the large-payload streaming configuration.
func (h *ConfigHandler) getLargePayloadConfig(ctx *fasthttp.RequestCtx) {
	if h.store.ConfigStore == nil {
		SendError(ctx, fasthttp.StatusServiceUnavailable, "config store not available")
		return
	}
	lpConfig, err := h.store.ConfigStore.GetLargePayloadConfig(ctx)
	if err != nil {
		SendError(ctx, fasthttp.StatusInternalServerError, fmt.Sprintf("failed to get large payload config: %v", err))
		return
	}
	if lpConfig == nil {
		// No persisted configuration yet - return the defaults so the UI
		// renders a sane starting point.
		SendJSON(ctx, configstoreTables.DefaultLargePayloadConfig)
		return
	}
	SendJSON(ctx, lpConfig)
}

// validateLargePayloadConfig enforces the server-side invariants: every byte
// value must be at least 1 KB and max_payload_bytes must be large enough to
// hold the configured request/response thresholds.
func validateLargePayloadConfig(c *configstoreTables.LargePayloadConfig) error {
	checks := []struct {
		name  string
		value int64
	}{
		{"request_threshold_bytes", c.RequestThresholdBytes},
		{"response_threshold_bytes", c.ResponseThresholdBytes},
		{"prefetch_size_bytes", c.PrefetchSizeBytes},
		{"max_payload_bytes", c.MaxPayloadBytes},
		{"truncated_log_bytes", c.TruncatedLogBytes},
	}
	for _, check := range checks {
		if check.value < minLargePayloadBytes {
			return fmt.Errorf("%s must be at least %d bytes (1 KB)", check.name, minLargePayloadBytes)
		}
	}
	if c.MaxPayloadBytes < c.RequestThresholdBytes {
		return errors.New("max_payload_bytes must be greater than or equal to request_threshold_bytes")
	}
	if c.MaxPayloadBytes < c.ResponseThresholdBytes {
		return errors.New("max_payload_bytes must be greater than or equal to response_threshold_bytes")
	}
	return nil
}

// updateLargePayloadConfig handles PUT /api/large-payload-config - Update the large-payload streaming configuration.
// Changes require a server restart to take effect.
func (h *ConfigHandler) updateLargePayloadConfig(ctx *fasthttp.RequestCtx) {
	if h.store.ConfigStore == nil {
		SendError(ctx, fasthttp.StatusServiceUnavailable, "config store not available")
		return
	}

	var payload configstoreTables.LargePayloadConfig
	if err := json.Unmarshal(ctx.PostBody(), &payload); err != nil {
		SendError(ctx, fasthttp.StatusBadRequest, fmt.Sprintf("invalid request format: %v", err))
		return
	}

	if err := validateLargePayloadConfig(&payload); err != nil {
		SendError(ctx, fasthttp.StatusBadRequest, err.Error())
		return
	}

	if err := h.store.ConfigStore.UpdateLargePayloadConfig(ctx, &payload); err != nil {
		logger.Warn("failed to save large payload configuration: %v", err)
		SendError(ctx, fasthttp.StatusInternalServerError, fmt.Sprintf("failed to save large payload configuration: %v", err))
		return
	}

	// Large-payload thresholds are read at startup, so a restart is required
	// for changes to take effect.
	if err := h.store.ConfigStore.SetRestartRequiredConfig(ctx, &configstoreTables.RestartRequiredConfig{
		Required: true,
		Reason:   "Large payload streaming configuration has been updated. A restart is required for changes to take effect.",
	}); err != nil {
		logger.Warn("failed to set restart required config: %v", err)
	}
	...
	SendJSON(ctx, map[string]any{
		"status":  "success",
		"message": "large payload configuration updated successfully",
	})
}
