# Loopback Gateway — Architecture & Contributor Guide

Loopback Gateway is a high-performance AI gateway: a fork of [maximhq/bifrost](https://github.com/maximhq/bifrost) that unifies 20+ LLM providers behind a single OpenAI-compatible API with ~11µs overhead at 5,000 RPS, and doubles as an MCP (Model Context Protocol) gateway that turns static chat models into tool-calling agents. The fork extends the upstream base with guardrails, RBAC, audit logging, SSO/SCIM, circuit breaking, and data connectors.

This document is the architecture and contributor reference for the whole codebase: how the repository is laid out, how a request flows through the system, how to implement providers and plugins, the invariants and gotchas that keep the gateway fast and correct, the testing infrastructure, and the frontend conventions. New contributors should read it before making non-trivial changes.

## Table of Contents

- [Relationship to Upstream Bifrost](#relationship-to-upstream-bifrost)
...
- [Architecture](#architecture)
  ...
  - [BifrostContext — Custom Context](#bifrostcontext--custom-context)
...
- [Fork Additions](#fork-additions)
  - [Fork Plugins](#fork-plugins)
  - [UI Additions](#ui-additions)
  - [Safety Invariants](#safety-invariants)
- [Gotchas](#gotchas)
- [Adding a New Provider — Full Checklist](#adding-a-new-provider--full-checklist)
- [Testing](#testing)
- [Common Workflows](#common-workflows)
- [Key Files Quick Reference](#key-files-quick-reference)
- [Code Style](#code-style)
- [Frontend Code Guidelines & Patterns](#frontend-code-guidelines--patterns)

---

## Relationship to Upstream Bifrost

Loopback Gateway is a rebranded and extended fork of `maximhq/bifrost`. Two decisions shape day-to-day work in this repository:

- **Go module paths are intentionally NOT renamed.** All modules remain `github.com/maximhq/bifrost/*` to keep upstream merges clean (see [ADR 0001 — Language and Base](adr/0001-language-and-base.md)). Do not rename import paths.
- **`go.work` is checked in and load-bearing.** It makes every sub-module build against the local `./core` checkout instead of the published `core` release, and carries `replace` directives for the fork plugins. `make setup-workspace` (run automatically by `make dev`) deletes and regenerates it — if you touch workspace setup, verify the fork plugin entries and `replace` directives survive.

Licensing: Apache-2.0 with attribution in `NOTICE`. Any code ported from MIT-licensed projects (e.g. Portkey, LiteLLM) needs an entry in `THIRD_PARTY_LICENSES.md` (see [ADR 0002 — Licensing and Attribution](adr/0002-licensing-and-attribution.md)).

A note on the planning documents at the repository root: they overlap and can conflict. Precedence, most-truthful first: `STATUS.md` (what actually builds and runs today) → `DELIVERY.md` (feature scorecard) → `FEATURE_MATRIX.md` / `PROJECT_PLAN.md` / `README.md` feature lists (aspirational roadmap, not shipped features). Do not describe roadmap items as existing capabilities.
...
│   ├── loopbackguard/             # Fork: guardrail engine + PII redaction (see Fork Additions)
│   ├── loopbackkafka/             # Fork: Kafka telemetry connector (see Fork Additions)
│   ├── prompts/                   # Fork: stored prompt template resolution (see Fork Additions)
│   └── modelcatalogresolver/      # Fork: unprefixed-model → provider lookup (see Fork Additions)
│
...
│
├── go.work                        # Go workspace — checked in, load-bearing (see Go Workspace)
...
Loopback Gateway is a **multi-module Go workspace**. Each module has its own `go.mod`:
...
├── cli/go.mod               # CLI module
└── plugins/*/go.mod         # 13 plugin modules (governance, telemetry, logging, loopbackguard, ...)
```

Rules:

- Run `go mod tidy` in the **specific module directory**, not the root.
- Cross-module imports resolve via the workspace locally, but need explicit `require` in `go.mod` for releases.
- The workspace requires **Go 1.26.5** (`go.work` directive). A Nix flake (`nix develop`) provides the toolchain.
- `go.work` is checked in and carries `replace` directives for the fork plugins (`loopbackguard`, `loopbackkafka`). `make setup-workspace` regenerates it — verify the fork entries survive if you change workspace setup.
...
```bash
# Development
make dev                                 # Full local dev (UI on :3000 + API with hot reload via air, proxied at :8080)
make build                               # Build bifrost-http binary (LOCAL=1 to build against go.work)
...
# Plugin tests
make test-plugins                        # All plugins (including loopbackguard)
make test-governance                     # Governance plugin specifically
make test-governance PATTERN=RateLimit   # Governance tests matching pattern
make test-framework                      # Framework module tests
make test-all                            # core + framework + plugins + transport + cli

# Fork plugin tests directly (unit tests, no live APIs needed)
...
`docker-compose.yml` provides the dev stack: `loopback-gateway` (:8080), `loopback-admin` nginx UI proxy (:8081), and optional `redis` and `postgres` services.
...
**Reserved context keys** (set by internals — do not set manually):
...
When in doubt: if a new ctx key would hold a slice/map that grows with request content, route the storage through a manager and keep only the ID in ctx.
...
**SDK integration layers** (`transports/bifrost-http/integrations/`) provide request/response converters between provider-native SDK formats and the internal format. This enables drop-in replacement of OpenAI SDK, Anthropic SDK, AWS Bedrock SDK, Google GenAI SDK, LangChain, and LiteLLM.
...
The UI is rebranded (Bifrost → Loopback Gateway across ~67 files) with new workspace pages under `ui/app/workspace/`: `guardrails/`, `pii-redactor/`, `circuit-breaker/`, `rbac/`, `scim/`, `audit-logs/`, `prompt-repo/`, `mcp-tool-groups/`, `observability/`.

### Safety Invariants

These invariants must not be broken by any change:

- **RBAC is fail-open** until explicitly enabled AND at least one role assignment exists; the local admin must always be allowed.
- **Circuit breaker registry is nil until configured** — with no config, the request path must remain byte-for-byte unchanged.
- **SSO/SCIM (Keycloak) is additive and default-off** — password auth must keep working.
...
When `BlockRestrictedWrites()` is active, writes to reserved keys (governance IDs, retry counts, fallback index, etc.) are **silently ignored** — no error. If a plugin needs to pass data through context, it should use its own custom key type.

### 12. `fasthttp`, Not `net/http`

Provider HTTP calls use `github.com/valyala/fasthttp`. The API is different from `net/http`:
...
### 19. Known Pre-existing Upstream Test Debt

The configstore full-migration test and governance secret-var API drift are known upstream test failures, not regressions introduced by this fork. Don't chase them as regressions.

---

## Adding a New Provider — Full Checklist

1. Create `core/providers/<name>/` with files per the pattern (see [Provider Implementation](#provider-implementation) above)
...
### Prefer `make test-core` over raw `go test` for provider-level tests
...
3. Add to `go.work`
4. Register in transport layer or gateway config
...
5. If E2E tests break, update the affected specs under `tests/e2e/features/` to match the new UI
...
| Guardrail engine (fork) | `plugins/loopbackguard/` |
| Kafka connector (fork) | `plugins/loopbackkafka/` |
...
This section defines the standards, structure, and best practices for writing frontend code in this project.
